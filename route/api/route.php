...
Route::any('wechat/notify', 'payservice.WxpayController/notify'); //微信app支付回调
Route::any('routine/notify', 'wechat.AuthController/notify'); //小程序支付回调
Route::any('wechat/pc_notify', 'payservice.WxpayController/notify'); //PC网站支付
Route::any('wechat/search_order', 'payservice.WxpayController/search_order'); //PC查询订单状态
Route::any('zhifubao/return_notify', 'payservice.AlipayController/ali_return_notify'); 
Route::any('alinotify', 'payservice.AlipayController/notify');
...
//会员授权接口
Route::group(function () {
    Route::get('pay_type', 'order.StoreOrderController/pay_type'); // 退出登录
    ...
    //物流列表
   Route::get('express', 'PublicController/express');
   //用户类  地址
    Route::get('address/detail/:id', 'user.UserController/address')->name('address'); //获取单个地址
    Route::post('express/set', 'user.UserController/express_set'); //物流信息设置
    Route::get('express/read/:id', 'user.UserController/express_read'); //物流信息设置
    Route::post('express/del/:id', 'user.UserController/express_del'); //物流信息设置
    ...
    Route::get('collect/list', 'user.UserController/collect_list')->name('collectUser'); //收藏产品列表
    Route::post('collect/add', 'user.UserController/collect_add')->name('collectAdd'); //添加/取消收藏
    ...
    //足迹
    Route::get('foot/list', 'user.UserController/foot_list'); //收藏产品列表
    ...
    Route::post('coupon/receive_bag', 'store.StoreCouponsController/receive_bag')->name('couponReceiveBag'); //领取优惠券礼包
    Route::post('coupon/receive/batch', 'store.StoreCouponsController/receive_batch')->name('couponReceiveBatch'); //批量领取优惠券
    Route::get('coupons/user', 'store.StoreCouponsController/user')->name('couponsUser'); //用户已领取优惠券
    ...
    Route::post('cart/saveall', 'store.StoreCartController/saveall'); //购物车批量添加
    Route::post('cart/del', 'store.StoreCartController/del')->name('cartDel'); //购物车删除
    Route::post('cart/select', 'store.StoreCartController/select'); //购物车选中状态
    ...
    Route::post('order/computepostage', 'order.StoreOrderController/computePostage')->name('computepostage'); //计算订单运费
    Route::post('order/create', 'order.StoreOrderController/create')->name('orderCreate'); //订单创建
    Route::get('order/business_rate', 'order.StoreOrderController/business_rate'); //订单创建
    Route::get('order/data', 'order.StoreOrderController/data')->name('orderData'); //订单统计数据
    Route::get('order/list', 'order.StoreOrderController/order_list')->name('orderList'); //订单列表
    Route::get('order/list_pc', 'order.StoreOrderController/list_pc')->name('orderLists'); //商城订单列表
    Route::get('order/detail/:id', 'order.StoreOrderController/detail')->name('orderDetail'); //订单详情
    Route::get('order/order_pay_state/:uni', 'order.StoreOrderController/order_pay_state')->name('orderDetail'); //查询微信支付结果
    ...
    Route::post('order/pay_amount', 'order.StoreOrderController/pay_amount'); //支付页面
    ...
    //用户申请成为商家
    Route::get('business_certification', 'user.UserController/business_certification')->name('BusinessCertification'); //用户申请成为商家
    Route::post('save_business_certification', 'user.UserController/save_business_certification')->name('saveBusinessCertification'); //申请成为商家并保存

    //用户消息
    Route::get('message', 'user.UserMessage/index')->name('UserMessage'); //获取用户消息

    //订阅活动通知
    Route::post('seckill/subscribe', 'activity.StoreSeckillController/subscribe'); //取消点赞

})->middleware(\app\http\middleware\AllowOriginMiddleware::class)->middleware(\app\http\middleware\AuthTokenMiddleware::class, true)->middleware(\app\http\middleware\DisableUserMiddleware::class);;

//未授权接口
Route::group(function () {

    //未读消息数量
    Route::get('msg/num', 'user.UserMessage/num');
    //店铺列表
    Route::post('shop/list', 'shop.ShopController/list');
    //店铺详情
    Route::get('shop/read', 'shop.ShopController/read');
    //系统文章
    Route::get('article/sys', 'publics.ArticleController/sys'); //文章列

    //联系客服信息
    Route::get('server_info', 'user.UserController/get_server_info')->name('serverInfo'); //联系客服信息

    //公共类
    Route::get('index', 'PublicController/index')->name('index'); //首页
    Route::any('index_pc', 'PublicController/index_pc')->name('index_pc'); //首页 - PC
    Route::get('search_hot_keywords', 'PublicController/search')->name('searchKeyword'); //热门搜索关键字获取
    Route::get('app_search', 'PublicController/app_search'); // 搜索商品及店铺
    //产品分类类
    Route::get('category', 'store.CategoryController/category')->name('category');
    //获取子类
     Route::get('subcate', 'store.CategoryController/subcate');
    //产品类
    ...
    Route::get('boutique_list', 'store.StoreProductController/boutique_list')->name('boutique_list'); //精品推荐商品 - 代销商品
    ...
    Route::post('product/consignment_product', 'store.StoreProductController/consignment_product')->name('consignment_product'); //代销商品
    //文章分类类
    ...
    Route::get('seckill/list', 'activity.StoreSeckillController/lst')->name('seckillList'); //秒杀产品列表
    ...
    //优惠券礼包
    Route::get('coupons_bag/index', 'store.StoreCouponsBagController/index')->name('couponsBagIndex'); //可领取优惠券列表
    Route::get('coupons_bag/coupon_list', 'store.StoreCouponsBagController/lst')->name('couponsBagList'); //礼包详情页
    ...
    Route::get('qiniu_token', 'PublicController/qiniu_token')->name('qiniu_token');


    //商家详情
    Route::get('business/detail', 'business.BusinessController/detail')->name('businessDetail');


    //商城介绍
    Route::get('about', 'publics.ArticleController/about')->name('about');

    //创建用户活动通知消息的定时任务
    Route::get('auto_task/set_activity_message', 'AutoTask/set_activity_message')->name('about');
    Route::get('auto_task/set_user_tag', 'AutoTask/set_user_tag')->name('setUserTag');


    //限时活动首页
    Route::get('activity/index', 'activity.StoreSeckillController/index')->name('activityIndex');
    Route::get('activity/activity_list', 'activity.StoreSeckillController/activity_list')->name('activityList');
    Route::get('activity/product_detail', 'activity.StoreSeckillController/detail')->name('activityProDetail');


    //绑定tenant_id
    Route::post('bind_tenant', 'PublicController/bind_tenant')->name('bindTenant');


    //异业适配商品列表
    Route::post('admin_index', 'admin_different_product.DifferentProductController/admin_index')->name('adminIndex');
    //异业适配商品详情
    Route::post('different_product_detail', 'admin_different_product.DifferentProductController/different_product_detail')->name('differentProductDetail');

    //异业适配商品加入购物车
    Route::post('different_cart/add', 'admin_different_product.DifferentProductController/add')->name('DifferentAdd');
    //异业适配购物车删除
    Route::post('different_cart/del', 'admin_different_product.DifferentProductController/del')->name('DifferentDel');
    //异业适配购物车修改产品数量
    Route::post('different_cart/num', 'admin_different_product.DifferentProductController/num')->name('DifferentNum');

    Route::post('different_cart/count', 'admin_different_product.DifferentProductController/num')->name('DifferentCount');

    //异业适配购物车商品列表
    Route::post('different_cart/list', 'admin_different_product.DifferentProductController/cart_list')->name('DifferentCartList');

    Route::post('different_cart/create/:key', 'admin_different_product.DifferentProductController/create')->name('DifferentCreate');

    Route::post('different_cart/again', 'order.StoreOrderController/again')->name('DifferentAgain'); //订单 再次下单

    Route::post('different_cart/pay', 'order.StoreOrderController/pay')->name('DifferentPay'); //订单支付

    Route::post('order/confirm', 'order.StoreOrderController/confirm')->name('orderConfirm'); //订单确认

    //申请代销
    Route::post('apply_consignment', 'admin_different_product.DifferentProductController/apply_consignment')->name('ApplyConsignment');

    Route::post('save_apply_consignment', 'admin_different_product.DifferentProductController/save_apply_consignment')->name('SaveApplyConsignment');


    Route::get('fans/get_staff_plan_info', 'FansController/get_staff_plan_info')->name('sendFansNote');

    Route::any('wechat/margin_notify', 'payservice.WxpayController/margin_notify')->name('margin_notify'); //商户支付保证金回调
