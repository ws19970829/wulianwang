...
class StoreCouponUser extends BaseModel
{
     ...
     * @return array
     */
    public static function getUsableCouponList($uid, $shop_id, $cartGroup, $price)
    {
        $list = [];

        foreach ($cartGroup['valid']['list'] as $value) {
            $lst1[] = self::alias('a')
        ...
        $lst3 = self::alias('a')
            ...
            ->hidden(['status', 'is_fail'])
            ->toArray();
        $list = empty($list) ? $lst3 : array_merge($list, $lst3);
    ...
    public static function useCoupon($issue_id, $uid)
    {
        // return self::where('id', $id)->update(['status' => 1, 'use_time' => time()]);
        return StoreCouponIssueUser::where([
            'issue_coupon_id' => $issue_id,
            'uid' => $uid
        ])->update(['is_use' => 1, 'use_time' => time()]);
    }
    ...
    public static function addUserCoupon($uid, $cid, $type = 'get')
    {
        ...
        $data['type'] = $type;
        $data['tenant_id'] = $couponInfo['tenant_id'];
