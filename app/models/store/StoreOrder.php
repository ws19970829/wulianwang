...
class StoreOrder extends BaseModel
{
    ...
    public function getCustomerMobileAttr($v, $data)
    {
        return SystemConfig::getConfigValue('server_tel', $data['tenant_id']);
    }
    ...
    protected function getGiftListAttr($value)
    {
        return $value ? json_decode($value, true) : [];
    }

    protected function getGiftCouponListAttr($value)
    {
        return $value ? json_decode($value, true) : [];
    }

    public function getArreasAttr($v, $data)
    {
        if ($data['order_type'] == 1) return '0.00';
        $price = bcsub($data['total_price'], $data['coupon_price'], 2);
        return bcsub($price, $data['business_rate'], 2);
    }
     ...
     * @placeholder [type]     status   [-1: 退款 -2: 退货成功 0: 待发货 1: 待收货 2: 已收货 3: 待评价 4: 已取消]
     * @return      [type]              [description]
     */
    public function getStatusTextAttr($v, $data)
    {
        if ($data['refund_status']) {
            if ($data['refund_status'] == 2) return '退款成功';
            if ($data['plat_verify']) {
                return '申请退款(平台审核中)';
            } else {
                return '申请退款(商家审核中)';
     ...
     * @return      [type]              [description]
     */
    public function getStatusValueAttr($v, $data)
    {
        if ($data['status'] == 4) return config('status.order_cancel');
        if (!$data['paid']) return config('status.order_un_pay'); // '未支付';
        switch ($data['status']) {
            case '-1':
                return '申请退款';
            case '-2':
                return '退款成功';
            case '0':
                return config('status.order_un_deliver'); // '待发货';
            case '1':
                return config('status.order_un_receive'); // '待收货';
            case '2':
                return config('status.order_completed'); // '已完成';
            case '4':
                return config('status.order_cancel'); // '已取消';
        }
    }

    public function getDiscountPriceTextAttr($v, $data)
    {
        $coupon_price = $data['coupon_price'] ?? 0;
        $pay_postage = $data['pay_postage'] ?? 0;
        return "其他金额：优惠券-{$coupon_price}";
    }

    /**计算邮费 */
    private static function compute_freight($cartInfo, $totalShopPrice, $addr)
    {
        $storePostage = 0;
        $storeShopPostage = [];
        $type = [];
        foreach ($cartInfo as $info) {
            $storeShopPostage[$info['id']] = 0;
            $type[$info['id']] = 0;
        }
        if ($addr) {
            ...
            $temp_num = [];
            foreach ($cartInfo as $info) {
                $storeFreePostage = trim(db('system_config')->where([
                    'menu_name' => 'store_free_postage',
                    'tenant_id' => $info['id']
                ])->value('value'), '""');
                if (empty($storeFreePostage)) continue;
                if (!empty($totalShopPrice[$info['id']]) && $storeFreePostage <= $totalShopPrice[$info['id']]) {
                    continue; //如果总价大于等于满额包邮 邮费等于0
                }
                foreach ($info['list'] as $cart) {
                    ...
                    if (!isset($temp_num[$info['id']][$cart['productInfo']['temp_id']])) {
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['number'] = $num;
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['price'] = bcmul($cart['cart_num'], $cart['truePrice'], 2);
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['first'] = $region['first'];
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['first_price'] = $region['first_price'];
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['continue'] = $region['continue'];
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['continue_price'] = $region['continue_price'];
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['temp_id'] = $cart['productInfo']['temp_id'];
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['city_id'] = $addr['city_id'];
                    } else {
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['number'] += $num;
                        $temp_num[$info['id']][$cart['productInfo']['temp_id']]['price'] += bcmul($cart['cart_num'], $cart['truePrice'], 2);
                    }
            ...
            foreach ($temp_num as &$v) {
                array_multisort(array_column($v, 'first_price'), SORT_DESC, $v);
            }
            ...
            foreach ($temp_num as $k => &$v) {
                foreach ($v as $k1 => &$v1) {
                    if (ShippingTemplatesFree::where('temp_id', $v1['temp_id'])->where('city_id', $v1['city_id'])->where('number', '<=', $v1['number'])->where('price', '<=', $v1['price'])->find()) {
                        unset($v[$k1]);
                    }
                }
            }
            ...
            foreach ($temp_num as $k => $va) {
                foreach ($va as $v) {
                    if ($type[$k] == 0) {
                        if ($v['number'] <= $v['first']) {
                            $storePostage = bcadd($storePostage, $v['first_price'], 2);
                            $storeShopPostage[$k] = bcadd($storeShopPostage[$k], $v['first_price'], 2);
                        } else {
                            if ($v['continue'] <= 0) {
                                ...
                                $storeShopPostage[$k] = bcadd(bcadd($storeShopPostage[$k], $v['first_price'], 2), bcmul(ceil(bcdiv(bcsub($v['number'], $v['first']), $v['continue'] ?? 0, 2)), $v['continue_price']), 2);
                            }
                        }
                        $type[$k] = 1;
                    } else {
                        if ($v['continue'] <= 0) {
                            ...
                            $storeShopPostage[$k] = bcadd($storeShopPostage[$k], bcmul(ceil(bcdiv($v['number'], $v['continue'] ?? 0, 2)), $v['continue_price']), 2);
                        }
                    }
                }
            }
        }
        return [$storePostage, $storeShopPostage];
    }
     ...
     * @return array
     */
    public static function getOrderPriceGroup($cartInfo, $addr, $tenant_id = 0)
    {
        $publicController = new PublicController();
        $publicController->getSysConfigValue('offline_postage', $tenant_id);

        [$totalPrice, $totalShopPrice] = self::getOrderSumPrice($cartInfo, 'truePrice'); //获取订单总金额
        [$costPrice, $costShopPrice] = self::getOrderSumPrice($cartInfo, 'costPrice'); //获取订单成本价
        // [$storePostage, $storeShopPostage] = self::compute_freight($cartInfo, $totalShopPrice, $addr);    //获取邮费

        $storePostage = 0;
        $storeShopPostage = [];

        return compact('storePostage', 'storeShopPostage', 'costShopPrice', 'costPrice', 'totalPrice', 'totalShopPrice');
    }
    ...
    public static function getOrderSumPrice($cartInfo, $key = 'truePrice')
    {
        ...
        foreach ($cartInfo as $info) {
            ...
            foreach ($info['list'] as $cart) {
                if ($key == 'truePrice') {
                    $SumPrice = bcadd($SumPrice, self::getTruePrice($cart), 2);
                    $total =  bcadd($total, self::getTruePrice($cart), 2);
                } else {
                    $SumPrice =  bcadd($SumPrice, bcmul($cart['cart_num'], $cart[$key], 2), 2);
                    $total =  bcadd($total, bcmul($cart['cart_num'], $cart[$key], 2), 2);
                }
            }
            $shopPrice[$info['id']] = $total;
        }
        return [$SumPrice, $shopPrice];
    }

    private static function getTruePrice($cart)
    {
        if (!empty($cart['combination_id']) || !empty($cart['seckill_id'])) {
            //秒杀和拼团商品的单价是原始单价
            $unit = $cart['productInfo']['price'];
        } else {
            if ($cart['cart_num'] >=  $cart['productInfo']['attrInfo']['discount']) {
                $unit = $cart['productInfo']['attrInfo']['discount_gt'];
            } else {
                $unit = $cart['productInfo']['attrInfo']['discount_lt'];
            }
        }
        return bcmul($unit, $cart['cart_num'], 2);
    }
    ...
    /**计算订单金额 */
    public static function computePrice($uid, $key, $shopinfo, $order_type)
    {
        ...
        try {
            ...
            foreach ($cartInfo as $info) {
                $cost = 0;
                $business_rate = 0;
                $payPrice = $priceGroup['totalShopPrice'][$info['id']];
                $couponInfo = [];
                foreach ($shopinfo as $s) {
                    if ($s['id'] != $info['id']) continue;
                    if (!empty($s['couponId'])) {
                        if (!$info['use_coupon']) {
                            return self::setErrorInfo('活动商品无法使用优惠券!');
                        }
                        if (!$couponId = StoreCouponIssue::beforeCreateOrder($uid, $s['couponId'])) return false;

                        $couponInfo = StoreCoupon::get($couponId);

                        if (empty($couponInfo))   return self::setErrorInfo('优惠券不存在!', true);
                    ...
                    foreach ($info['list'] as $cart) {
                        ...
                        //验证商家券使用的商品是否合法
                        if (!empty($couponInfo)) {
                            if (empty($couponInfo['category_id']) || $couponInfo['type'] != 1) continue;

                            //商家券后台设置了商品类别需要验证，没设置无需验证
                            $ids = StoreProduct::where('id', $cart['product_id'])->value('cate_id');
                            $temp = explode(',', $ids);
                            if (!in_array($couponInfo['category_id'], $temp)) {
                                return self::setErrorInfo('订单中含有优惠券无法使用的商品', true);
                            }
                        }
                    }
                    //物流代收或者欠款需付手续费
                    ...
                    if ($order_type != 1) {
                        ...
                        if (empty($business_rate)) return self::setErrorInfo('未配置手续费率!', true);
                        $cost = bcmul($business_rate / 100, $payPrice, 2);
                        $payPrice = $cost;
                    }
                }

                // $postage = $priceGroup['storeShopPostage'][$info['id']];
                // $total = bcadd($total, $payPrice + $postage, 2);
            }
            return $total;
        } catch (\Exception $e) {
            return self::setErrorInfo($e->getMessage() . $e->getLine() . $e->getFile());
        }
    }


    /**创建订单 */
    public static function cacheKeyCreateOrder($uid, $key, $addressId, $shopinfo, $order_type)
    {
        /**
         * 1.获取购物车信息   
         * 2.验证合法性
         * 3.根据cartinfo拆单
         * 4.拆单时组装优惠券和备注(shopinfo)，重新计算每笔订单的价格
         * 5.生成订单，把订单id返回给前端
         */
        ...
        try {
            ...
            $cartInfo = $cartGroup['cartInfo'];
            $priceGroup = $cartGroup['priceGroup'];
            $addressInfo = UserAddress::with('express')
                ->where('uid', $uid)
                ->where('id', $addressId)
                ->find();
            if (empty($addressInfo)) return self::setErrorInfo('地址选择有误!', true);
            if (empty($addressInfo['express_id'])) return self::setErrorInfo('物流公司不存在', true);
            $pay_type = 'weixin';
            ...
            foreach ($cartInfo as $info) {
                ...
                foreach ($shopinfo as $s) {
                    ...
                    if (!empty($s['couponId'])) {
                        if (!$info['use_coupon']) {
                            return self::setErrorInfo('活动商品无法使用优惠券!', true);
                        }
                        if (!$couponId = StoreCouponIssue::beforeCreateOrder($uid, $s['couponId'])) return false;
                        ...
                        $couponInfo = StoreCoupon::get($couponId);
                        if (empty($couponInfo))   return self::setErrorInfo('优惠券不存在!', true);
                        $type = StoreCoupon::type($couponId);
                        if ($couponInfo['type'] == 0) {
                            //商家返还平台券金额
                            $give_store_cash = bcadd($couponInfo['coupon_price'], $give_store_cash, 2);
                        }
                        $issue_id = $s['couponId'];
                        ...
                        if ($payPrice < 0) $payPrice = 0;
                        //代表已使用了优惠券
                        $res1 = StoreCouponUser::useCoupon($s['couponId'], $uid);
                ...
                foreach ($info['list'] as $cart) {
                    ...
                    if (!self::checkProductStock($uid, $cart['product_id'], $cart['cart_num'], $cart['product_attr_unique'], $cart['combination_id'], $cart['seckill_id'], $cart['bargain_id'], 0, 0, false)) {
                        return false;
                    }
                    ...
                    //减库存加销量
                    //减库存加销量
                    if (!empty($cart['combination_id'])) {
                        $combination_id[] = $cart['combination_id'];
                        $res5 = $res5 && StoreCombination::decCombinationStock($cart['cart_num'], $cart['combination_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                    } else if (!empty($cart['seckill_id'])) {
                        $seckill_id[] = $cart['seckill_id'];
                        $res5 = $res5 && StoreSeckill::decSeckillStock($cart['cart_num'], $cart['seckill_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                    } else {
                        $res5 = $res5 && StoreProduct::decProductStock($cart['cart_num'], $cart['product_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                    }
                }
                //物流代收或者欠款需付定金
                if ($order_type != 1) {
                    ...
                    $cost = bcmul($business_rate / 100, $payPrice, 2);
                    $payPrice = $cost;
                    $give_store_cash = bcadd($payPrice, $give_store_cash, 2);
                } else {
                    //$payPrice = bcadd($payPrice, $postage, 2);
                    $give_store_cash = bcadd($payPrice, $give_store_cash, 2);
                    $payPrice = bcadd($payPrice, $cost, 2);
                }
                ...
                $orderInfo = [
                    ...
                    'is_consignment' => 0,
                    'admin_id' => $userInfo['admin_account_id'],
                    'order_id' => $order_id,
                    ...
                    'cart_id' => $cartIds,
                    'total_num' => $totalNum,
                    'total_price' => $priceGroup['totalShopPrice'][$info['id']],
                    ...
                    'combination_id' => implode(',', $combination_id),
                    'pink_id' => 0,
                    'seckill_id' => implode(',', $seckill_id),
                    'bargain_id' => 0,
                    'cost' => $priceGroup['costShopPrice'][$info['id']],
                    'is_channel' => 1,
                    'add_time' => time(),
                    'unique' => md5($key . '_' . $order_id),
                    'express_id' => $addressInfo['express']['id'],
                    'give_store_cash' => $give_store_cash
                ];
                ...
                //保存购物车商品信息
                $res4 = false !== StoreOrderCartInfo::setCartInfo($order['id'], $info);
            ...
            if (empty($orders) || count($orders) != count($cartInfo)) {
                return self::setErrorInfo('系统繁忙订单生成失败', true);
            }

            self::commitTrans();
            return $orders;
        } catch (\PDOException $e) {
            self::setErrorInfo('生成订单时SQL执行错误错误原因：' . $e->getMessage());
            self::rollbackTrans();
            return false;
        } catch (\Exception $e) {
            self::setErrorInfo('生成订单时系统错误错误原因：' . $e->getMessage() . $e->getFile() . $e->getLine());
            self::rollbackTrans();
            return false;
        }
    }
     ...
     * @Desc    计算代销订单利润
     */
    public static function calculateOrderProfit($cartlist = array())
    {
        $profit = 0;
        foreach ($cartlist as $k => $v) {
            $org_product = StoreProductAttrValue::where('unique', $v['productInfo']['attrInfo']['org_product_unique'])->find();
            $profit += ($v['productInfo']['attrInfo']['price'] - $org_product['price']) * $v['cart_num'];
        }
    ...
    public static function RegressionStock($order)
    {
        ...
        $res5 = true;
        $cartInfo = StoreOrderCartInfo::where('cart_id', 'in', $order['cart_id'])
            ->select()
            ->toArray();
        foreach ($cartInfo as $cart) {
            //增库存减销量
            if (!empty($cart['cart_info']['combination_id'])) {
                $res5 = $res5 && StoreCombination::incCombinationStock($cart['cart_info']['cart_num'], $cart['cart_info']['combination_id'], isset($cart['cart_info']['productInfo']['attrInfo']) ? $cart['cart_info']['productInfo']['attrInfo']['unique'] : '');
            } elseif (!empty($cart['cart_info']['seckill_id'])) {
                $res5 = $res5 && StoreSeckill::incSeckillStock($cart['cart_info']['cart_num'], $cart['cart_info']['seckill_id'], isset($cart['cart_info']['productInfo']['attrInfo']) ? $cart['cart_info']['productInfo']['attrInfo']['unique'] : '');
            } else {
                $res5 = $res5 && StoreProduct::incProductStock($cart['cart_info']['cart_num'], $cart['cart_info']['product_id'], isset($cart['cart_info']['productInfo']['attrInfo']) ? $cart['cart_info']['productInfo']['attrInfo']['unique'] : '');
            }
        }
        return $res5;
    }
    ...
    public static function RegressionCoupon($order)
    {
        if ($order['paid'] || $order['status'] == -2 || $order['is_del']) return true;
        $res = true;
        if ($order['issue_id'] && $order['coupon_id'] && StoreCouponIssueUser::be(['uid' => $order['uid'], 'issue_coupon_id' => $order['issue_id'], 'is_use' => 1])) {
            $res = $res && false !== StoreCouponIssueUser::where('uid', $order['uid'])
                ->where('issue_coupon_id', $order['issue_id'])
                ->where('is_use', 1)
                ->update(['is_use' => 0, 'use_time' => 0]);
        }
     ...
     * @throws \think\exception\DbException
     */
    public static function cancelOrder($id, $uid)
    {
        $order = self::where('order_id', $id)->where('uid', $uid)->find();
        ...
        try {
            ...
            self::rollbackTrans();
            return self::setErrorInfo(['message' => $e->getMessage() . $e->getLine() . $e->getFile()]);
        }
    }
    ...
    public static function orderApplyRefund($uni, $uid, $refundReasonWap = '', $refundReasonWapExplain = '', $refundReasonWapImg = [])
    {
        ...
        if ($order['status'] == 2) return self::setErrorInfo('不支持线上退款，请联系商家!');
        self::beginTrans();
        ...
        if (!$res) {
            ...
            (new UserMessage)->create_message($uid, '提示', '订单号:' . $order['order_id'] . '已发出退款申请，请等待商家审核！');
            return true;
        }
        // if (!$res)
        //     return self::setErrorInfo('申请退款失败!');
     ...
     * @return bool
     */
    public static function paySuccess($out_trade_no, $paytype = 'weixin', $formId = '')
    {
        //if (cache($out_trade_no)) return true;
        try {

            $ids = OrderOutTradeNo::getOrderId($out_trade_no);
            foreach ($ids as $v) {
                //修改订单状态
                $order = self::where('id', $v)->find();
                $order->paid = 1;
                $order->pay_type = $paytype;
                $order->pay_time = time();
                $order->save();
                //增加店铺账户余额
                $tenant = SystemAdmin::get($order->tenant_id);
                $balance = bcadd($order->give_store_cash, $tenant->balance);
                $tenant->balance = $balance;
                $tenant->save();
                // 记录支付明细
                $coupon_type = StoreCoupon::type($order->coupon_id);
                $now_money = SystemAdmin::where('id', $order->tenant_id)->value('balance');

                if ($order->business_rate != '0.00') {
                    //平台收入定金
                    $content = '定金预付' . floatval($order['business_rate']) . '元';
                } else {
                    $content  = '订单收入' . floatval($order['pay_price']) . '元';
                }
                //商户购买商品收入
                UserBill::income('待结算金额', $order->uid, 'now_money', 'pay_money', $order->pay_price, $order['id'], $now_money, $content, 1, $order->tenant_id);
                //平台订单收入
                UserBill::income('收入', $order->uid, 'now_money', 'admin_income', $order->pay_price, $order['id'], $now_money, $content, 1, 0);

                if ($order->coupon_price > 0) {
                    if ($coupon_type == 0) {
                        //平台优惠券发放支出
                        UserBill::expend('支出 ', $order->uid, 'now_money', 'admin_expend', $order->coupon_price, $order['id'], 0, '平台券发放' . floatval($order['coupon_price']) . '元', 1, 0);
                        //商户平台券收入
                        UserBill::income('待结算金额', $order->uid, 'now_money', 'pay_money', $order->coupon_price, $order['id'], $now_money,  '平台券收入' . floatval($order['coupon_price']) . '元', 1, $order->tenant_id);
                    } else {
                        //商家券支出
                        // UserBill::expend('商户优惠券', $order->uid, 'now_money', 'coupon', $order->coupon_price, $order['id'], 0, '商户优惠券发放' . floatval($order['coupon_price']) . '元', 1, 0);
                        UserBill::expend('待结算金额', $order->uid, 'now_money', 'pay_money', $order->coupon_price, $order['id'], 0, '商家优惠券发放' . floatval($order['coupon_price']) . '元', 1, $order->tenant_id);
                    }
                }

                // if (!empty($order->business_rate)) {

                //     //平台收入手续费
                //     UserBill::income('定金', $order->uid, 'now_money', 'service_charge', $order->business_rate, $order['id'], 0, '定金预付' . floatval($order['business_rate']) . '元', 1, 0);
                // }
                // if (!empty($order->pay_postage)) {
                //     //运费
                //     UserBill::income('运费', $order->uid, 'now_money', 'pay_postage', $order->pay_postage, $order['id'], 0, '运费' . floatval($order['pay_postage']) . '元', 1, 0);
                //     UserBill::income('运费', $order->uid, 'now_money', 'pay_postage', $order->pay_postage, $order['id'], 0, '运费' . floatval($order['pay_postage']) . '元', 1, $order->tenant_id);
                // }
            }
            //  cache($out_trade_no, true, 300);
            //支付成功后
            event('', [$order, $formId]);

            //如果该订单有赠送优惠券，则给该用户发放对应的优惠券
            // if ($order['gift_coupon_list']) {
            //     $tenant_id = User::getTenantIDbyUID($order['uid']);
            //     $coupon_user_data = [];
            //     foreach ($order['gift_coupon_list'] as $val) {
            //         $temp = [
            //             'cid' => $val['coupon_id'],
            //             'uid' => $order['uid'],
            //             'coupon_title' => $val['title'],
            //             'coupon_price' => $val['coupon_price'],
            //             'use_min_price' => $val['use_min_price'],
            //             'add_time' => time(),
            //             'end_time' => time() + 86400 * $val['coupon_time'],
            //             'type' => 'gift',
            //             'status' => 0,
            //             'is_fail' => 0,
            //             'tenant_id' => $tenant_id
            ...
            //         ];
            //         $coupon_user_data[] = $temp;
            //     }
            //     (new StoreCouponUser())->saveAll($coupon_user_data);
            // }
        } catch (\Throwable $th) {
            trace($th->getLine() . $th->getFile() . $th->getMessage(), 'error');
            return false;
        }
        return true;
    }
    ...
    {
        return self::with('express')->where('order_id|unique', $key)
            ->where('uid', $uid)
            ->where('is_del', 0)
            ->find();
    }
    ...
    public static function orderTakeAfter($order)
    {
        ...
        //     ]);
        // }
        (new UserMessage)->create_message($order['uid'], '通知', '亲，您的订单已收货，感谢您的光临！');
    }
    ...
    public static function takeOrder($uni, $uid)
    {
        ...
        if (
            ...
            try {
                ...
                UserBill::expend('待结算金额', $order->uid, 'now_money', 'pay_money', $order->give_store_cash, $order['id'], 0, '用户收货-' . $order->give_store_cash, 1, $order->tenant_id);

                UserBill::income('可用金额', $order->uid, 'now_money', 'balance', $order->give_store_cash, $order['id'], 0, '用户收货+' . $order->give_store_cash, 1, $order->tenant_id);
            } catch (\Exception $e) {
                self::rollbackTrans();
                return self::setErrorInfo($e->getMessage());
            }
    ...
    public static function tidyOrder($order, $detail = false, $isPic = false)
    {
        if ($detail == true && isset($order['id'])) {
            ...
            $shop = array_filter(array_unique(array_column($info, 'shop_id'), SORT_REGULAR));
            $order['shop'] = $shop;
            $order['shop_name'] = $shop[0]['real_name'] ?? '';
            $order['cartInfo'] = $info;
        }
        $status = [];
    ...
    public static function statusByWhere($status, $uid = 0, $model = null)
    {
        ...
        if ('' === $status)
            ...
            return $model->where('paid', 1)->where('status', 2);
        else if ($status == 4) //已取消
            return $model->where('status', 4);
     ...
     * @return StoreOrder|null
     */
    public static function statusValueByWhere($status, $uid = 0, $model = null)
    {
        //0未支付 1待发货 2待收货 3已完成 4已取消
        if ($model == null) $model = new self;
        switch ($status) {
            case config('status.order_un_all'):
                // 全部
                return $model;
            case config('status.order_un_pay'):
                // 未支付
                return $model->where('paid', 0)->where('status', 0)->where('refund_status', '0');
            case config('status.order_un_deliver'):
                // 待发货
                return $model->where('paid', 1)->where('status', 0)->where('refund_status', '<>', '2');
            case config('status.order_un_receive'):
                // 待收货
                return $model->where('paid', 1)->where('status', 1)->where('refund_status', '<>', 2);
            case config('status.order_completed'):
                // 已完成
                return $model->where('paid', 1)->where('status', 2);
            case config('status.order_cancel'):
                // 已取消
                return $model->where('status', 4);
            default:
                return $model;
    ...
    {
        if ($page) {
            $list = self::statusValueByWhere($status, $uid)
                ->where('is_del', 0)
                ->where('uid', $uid)
                ->field('add_time, seckill_id, bargain_id, combination_id, id, order_id, pay_price, total_num, total_price,
                    pay_postage, total_postage, paid, status, refund_status, pay_type, coupon_price, deduction_price,
                    pink_id, delivery_type, is_del, shipping_type, store_id,plat_verify,tenant_id')
                ->order('add_time DESC')
                ->page((int) $page, (int) $limit)
                ->select()
                ->append(['status_value', 'status_text', 'discount_price_text', 'customer_mobile'])
                ->toArray();
        } else {
            $list = self::statusValueByWhere($status, $uid)
                ->where('is_del', 0)
                ...
                ->field('add_time, seckill_id, bargain_id, combination_id, id, order_id, pay_price, total_num, total_price,
                    pay_postage, total_postage, paid, status, refund_status, pay_type, coupon_price, deduction_price,
                    pink_id, delivery_type, is_del, shipping_type,plat_verify,tenant_id')
                ->order('add_time DESC')
                ->page((int) $page, (int) $limit)
                ->select()
                ->append(['status_value', 'status_text', 'discount_price_text', 'customer_mobile'])
    ...
    {
        $order = self::where('uid', $uid)->where('order_id', $order_id)->where('is_del', 0)->field('seckill_id,bargain_id,combination_id,id,order_id,pay_price,total_num,total_price,pay_postage,total_postage,paid,status,refund_status,pay_type,coupon_price,deduction_price,delivery_type,shipping_type,tenant_id')
            ->order('add_time DESC')->append(['customer_mobile'])->find();
        if (!$order)
            return false;
        else
    ...
    public static function getPcUserOrderSearchList($uid, $type, $page, $limit, $search)
    {
        if ($search) {
            $order = self::searchUserOrder($uid, $search) ?: [];
            $list = $order == false ? [] : [$order];
            $count = $order == false ? 0 : 1;
        } else {
            $list = self::getUserOrderList($uid, $type, $page, $limit);

            if ($page) $count = self::statusByWhere($type, $uid)->where('is_del', 0)->where('uid', $uid)->count();
            else
                $count = self::statusByWhere($status, $uid)->where('is_del', 0)->where('uid', $uid)->count();
        }
        ...
        foreach ($list as $k => $order) {
            $list[$k] = self::tidyOrder($order, true);
            $list[$k]['system_store'] = SystemStore::getStoreDispose($order['store_id']);
            if ($list[$k]['_status']['_type'] == 3) {
                foreach ($order['cartInfo'] ?: [] as $key => $product) {
                    $list[$k]['cartInfo'][$key]['is_reply'] = StoreProductReply::isReply($product['unique'], 'product');
                    $list[$k]['cartInfo'][$key]['add_time'] = isset($product['add_time']) ? date('Y-m-d H:i', $product['add_time']) : '时间错误';
        ...
        return ['count' => $count, 'totalPage' => $totalPage, 'list' => $list];
    }
    ...
    {
        //全部数量
        $data['all_count'] = self::where('is_del', 0)->where('uid', $uid)->count();
        //订单支付没有退款 数量
        ...
        //订单待发货 数量
        $data['unshipped_count'] = self::where('paid', 1)
            ->whereIn('status', '0')
            ->where('is_del', 0)
            ->where('uid', $uid)
            ...
            ->count();
        //订单待收货 数量
        $data['unreceived_count'] = self::where('paid', 1)
            ->whereIn('status', '1')
            ->where('is_del', 0)
            ->where('uid', $uid)
            ->where('refund_status', '<>', 2)
        ...
        //订单已完成 数量
        $data['complete_count'] = self::where('status', 2)
            ->where('is_del', 0)
            ->where('uid', $uid)
            ->count();
        //订单退款
     ...
     * @param int $activity_product_id
     * @return bool
     */
    public static function checkProductStock($uid, $product_id, $cart_num = 1, $product_attr_unique = '', $combination_id = 0, $seckill_id = 0, $bargain_id = 0, $activity_id = 0, $activity_product_id = 0, $activity_unique = true)
    {
        ...
        if ($activity_id) {
            $activity_info = Activity::checkStatus($activity_id);
            if (!$activity_info) return self::setErrorInfo('活动已结束');

            $StoreSeckillinfo = ActivityProduct::getValidProductByProductyIDandActivityID($activity_id, $product_id);
            if (!$StoreSeckillinfo)
                return self::setErrorInfo('该产品已下架或删除');

            $buyLimit = Activity::getBuyLimitByActivityID($activity_id);
            ...
            if ($buyLimit > 0) {
                //TODO:判断限购次数
                $userbuycount = StoreOrder::where('uid', $uid)
                    ->where('paid', 1)
                ...
                //折扣和秒杀活动的每人限购数量
                if ($type == 4 || $type == 5) {
                    if ($buyLimit <= $userbuycount || $buyLimit < $cart_num)
                        return self::setErrorInfo('每人限购' . $StoreSeckillinfo['num'] . '件');
                }
            }

            $res = StoreProductAttrValue::where('product_id', $product_id)
                ->where('unique', $product_attr_unique)
                ->where('type', $type)
                ->field('suk,quota')
                ->find();

            $product_stock = StoreProductAttrValue::where('product_id', $StoreSeckillinfo['product_id'])
                ->where('suk', $res['suk'])
                ->where('activity_id', $activity_id)
                ->where('type', $type)
            ...
            $suk = StoreProductAttrValue::where('type', 1)
                ->where('unique', $product_attr_unique)
                ->value('suk') ?? "";
            // if ($activity_unique) {
            //     if (!\app\models\store\Activity::allow(0, $uid, $seckill_id)) {
            //         return self::setErrorInfo('您已经参与过秒杀了');
            //     }
            // }

            if (!StoreSeckill::forbidden($seckill_id)) {
                return self::setErrorInfo('该活动已被管理员禁用');
            }

            if (!StoreSeckill::isSeckillEnd($seckill_id))
                return self::setErrorInfo('不在秒杀活动时间');
            ...
            $res = StoreProductAttrValue::where('product_id', $product_id)
                ->where('unique', $product_attr_unique)
                ->where('type', 1)
                ->where('activity_id', $seckill_id)
                ->field('suk,quota')
                ->find();
            if ($cart_num > $res['quota'])
                return self::setErrorInfo('秒杀规格' . $suk . '数量不足，还剩' . $res['quota'] . '个');
            ...
            if ($product_stock < $cart_num)
                return self::setErrorInfo('秒杀规格' . $suk . '库存不足' . $cart_num);
        } elseif ($bargain_id) {
            ...
            // if ($activity_unique) {
            //     if (!\app\models\store\Activity::allow(1, $uid, $combination_id)) {
            //         return self::setErrorInfo('您已经参与过拼团了');
            //     }
            // }
            $StoreCombinationInfo = StoreCombination::getCombinationOne($combination_id);
            if (!$StoreCombinationInfo) return self::setErrorInfo('拼团已结束或已下架');
            $suk = StoreProductAttrValue::where('type', 3)
                ->where('unique', $product_attr_unique)
                ->value('suk') ?? "";
            ...
            $res = StoreProductAttrValue::where('product_id', $product_id)->where('unique', $product_attr_unique)->where('type', 3)->where('activity_id', $combination_id)->field('suk,quota')->find();

            if ($cart_num > $res['quota']) {
                return self::setErrorInfo('团购规格' . $suk . '数量不足,还剩' . $res['quota'] . '个');
            }
            ...
            if ($product_stock < $cart_num) {
                return self::setErrorInfo('团购规格' . $suk . '库存不足' . $cart_num);
            }
        } else {
            ...
            $attr = StoreProductAttrValue::where('type', 0)
                ->where('unique', $product_attr_unique)
                ->find() ;
            if(empty($attr)) return self::setErrorInfo('商品属性不存在');
            if(!$attr['advance_sale']){
                if (StoreProduct::getProductStock($product_id, $product_attr_unique) < $cart_num) {
                    return self::setErrorInfo('商品规格' . $attr['suk'] . '库存不足' . $cart_num);
                }
            }  
        }
        return true;
    ...
    public static function beforeCreateOrder($ids)
    {
        $pay_price = 0;
        $ids = explode(',', $ids);
        ...
        foreach ($ids as $v) {
            $order = self::get($v);
            if (empty($order)) {
                self::setErrorInfo('订单不存在');
                return false;
            }
            if ($order->paid != 0) {
                self::setErrorInfo('订单已支付');
                return false;
            }
            if ($order->is_del) {
                self::setErrorInfo('无效订单');
                return false;
            }
            $pay_price = bcadd($pay_price, $order->pay_price, 2);
        }
        return $pay_price;
    }
}
