...
class User extends BaseModel
{
    ...
    public static function getTenantIDbyUID($uid)
    {
        return self::where('uid', '=', $uid)->value('tenant_id');
    }

    public static function getTenantIDbyOpenID($openid)
    {
        $uid = WechatUser::where('openid', '=', $openid)->value('uid');
        if (!$uid) {
            return 0;
        }
        return self::where('uid', '=', $uid)->value('tenant_id');
    ...
    public static function backOrderBrokerage($orderInfo, bool $open = true)
    {
        ...
        if (isset($orderInfo['activity_id']) && $orderInfo['activity_id']) return true;
        ...
        //如果用户不存在，或者订单不是推广订单，则返回
        if (!$userInfo || !$orderInfo['spread_id']) return true;

        //        if (!User::be(['uid' => $userInfo['spread_uid'], 'is_promoter' => 1])) return self::backOrderBrokerageTwo($orderInfo, $open,$tenant_id);


        $cartId = is_string($orderInfo['cart_id']) ? json_decode($orderInfo['cart_id'], true) : $orderInfo['cart_id'];
        $brokeragePrice = StoreProduct::getProductBrokerage($cartId, true, $tenant_id);

        //TODO 获取上级推广员信息
        $spread_uid = $orderInfo['spread_id']; //上级推广员，从订单中获取，每笔订单单独返佣
        ...
        $spreadUserInfo = User::getUserInfo($spread_uid);
        //TODO 上级推广员返佣之后的金额
        ...
        //TODO 添加推广记录
        $res1 = UserBill::income('获得推广佣金', $spread_uid, 'now_money', 'brokerage', $brokeragePrice, $orderInfo['id'], $balance, $mark, 1, $tenant_id);
        //TODO 添加用户余额
        $res2 = self::bcInc($spread_uid, 'brokerage_price', $brokeragePrice, 'uid');
        $res2 = self::bcInc($spread_uid, 'now_money', $brokeragePrice, 'uid');
        //TODO 一级返佣成功 跳转二级返佣
        //        $res = $res1 && $res2 && self::backOrderBrokerageTwo($orderInfo, $open,$tenant_id);
        $res = $res1 && $res2; //不再进行二级返佣
     ...
     * @throws \think\exception\DbException
     */
    public static function backOrderBrokerageTwo($orderInfo, bool $open = true, $tenant_id = 0)
    {
        //TODO 获取购买商品的用户
        $userInfo = User::getUserInfo($orderInfo['uid']);
        //TODO 获取上推广人
        ...
        $cartId = is_string($orderInfo['cart_id']) ? json_decode($orderInfo['cart_id'], true) : $orderInfo['cart_id'];
        $brokeragePrice = StoreProduct::getProductBrokerage($cartId, false, $tenant_id);
        //TODO 返佣金额小于等于0 直接返回不返佣金
        if ($brokeragePrice <= 0) return true;
        //TODO 获取上上级推广员信息
        ...
        $open && self::beginTrans();
        //TODO 添加返佣记录
        $res1 = UserBill::income('获得推广佣金', $userInfoTwo['spread_uid'], 'now_money', 'brokerage', $brokeragePrice, $orderInfo['id'], $balance, $mark, 1, $tenant_id);
        //TODO 添加用户余额
    ...
    public static function register($account, $password, $spread)
    {
        ...
        $data['nickname'] = '会员' . substr($account, -4);
        $data['avatar'] = app('request')->domain() . '/static/img/head.jpg';
        $data['city'] = '';
        $data['language'] = '';
        $data['province'] = '';
        $data['country'] = '';
     ...
     */
    public static function getAllUserIds($tenant_id)
    {
        $user_ids = self::where('tenant_id', '=', $tenant_id)->field('uid,tenant_id')->select();
        if (count($user_ids) > 0) {
            $user_ids = $user_ids->column('uid');
            return $user_ids;
        }
        return [];
    }
     ...
     * phone 手机号
     * code 系统编码
     * tpl_data 要发送的数据 json 字符串
     */
    function send($phone, $tpl_data = '', $tpl_code = 'SMS_205340258')
    {
        //SMS_205397069是秒杀活动的
        //SMS_205884087是发送邀请码的
        $access_key_id = 'LTAI4G7WRAFbHcej1WQdkDc4';
        $access_key_secret = 'MZMlmuQ27xDcA0C1WYs4yjb3hWupjn';
        $sign_name = '迪远五金商城';
        ...
        if (!empty($tpl_data)) {
            ...
            if ($tpl_code == 'SMS_205884087') {
                $params['TemplateParam'] = ['code' => $tpl_data];
            } else {
                $params['TemplateParam'] = $tpl_data;
            }
        }
    ...
    /**验证短信验证码 */
    public function validate_sms($phone, $code, $return_type = 'json')
    {
