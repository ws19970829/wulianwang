...
<script>
    ...
    layList.form.render();
    //加载列表
    layList.tableList('List',"{:Url('user.user_tag/get_list')}",function (){
        return [
            {type: 'checkbox'},
            {field: 'id', title: 'ID', sort: true,event:'id'},
            {field: 'title', title: '标签名称',templet:'#store_name'},
            {field: 'is_auto', title: '优惠券类型',templet:function (d) {
                if(d.is_auto==1){
                    return '自动标签';
                }
                if(d.is_auto==0){
...
//        $(window.parent.document).find("#product").html("8888888");
        ...
        list.ids = ids;
        $.ajax({
            url:"{:Url('save_cache_tag')}?checked_ids={$ids}",
            data:list,
            type:'post',
            dataType:'json',
            success:function(re){
