...
<input type="hidden"  id="gift_json_input" value="{$gift_list_json}">
<div class="row">
...
<!--                            <input onclick="set_type(2)" type="radio" name="limit_buy_type" id="limit_buy_type2" class="layui-radio" value="2" {eq name="news['limit_buy_type']" value="2"}checked{/eq}><label for="limit_buy_type2">&nbsp;&nbsp;前几件享受支付有礼</label>-->
<!--                        </div>-->
<!---->
<!--                    </div>-->
<!--                </div>-->
...
<!--                    <label class="layui-form-label">每人每种商品前几件享受支付有礼</label>-->
<!--                    <div class="layui-input-inline" style="width: 200px;">-->
...
<!--                </div>-->
                ...
                <div class="form-group">
                    <div class="col-md-12">
                        <div class="col-md-12">
                            <label style="color:#aaa">权益设置</label>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <input onclick="set_type(1)" type="radio" name="gift_type" id="gift_type1" class="layui-radio" value="1" {eq name="news['gift_type']" value="1"}checked{/eq}><label for="gift_type1">&nbsp;&nbsp;送优惠券</label>&nbsp;&nbsp;&nbsp;&nbsp;
                            <input onclick="set_type(2)" type="radio" name="gift_type" id="gift_type2" class="layui-radio" value="2" {eq name="news['gift_type']" value="2"}checked{/eq}><label for="gift_type2">&nbsp;&nbsp;送商品</label>&nbsp;&nbsp;&nbsp;&nbsp;
                        </div>
                ...
                <div class="layui-btn-container dis_coupon {if $news.gift_type!=1}hide{/if}">
                    <!--                        <a class="layui-btn layui-btn-sm" href="{:Url('index')}">分类首页</a>-->
                    <button type="button" class="layui-btn layui-btn-sm" onclick="addCoupon()">选择优惠券</button>
                </div>
                <table class="layui-table dis_coupon {if $news.gift_type!=1}hide{/if}">
                    <colgroup>
                        <col width="150">
                        <col width="200">
                        <col>
                ...
                <div class="layui-btn-container dis_product_gift  {if $news.gift_type!=2}hide{/if}">
                    <!--                        <a class="layui-btn layui-btn-sm" href="{:Url('index')}">分类首页</a>-->
                    <button type="button" class="layui-btn layui-btn-sm" onclick="addGift()">添加赠品</button>
<!--                    {if count($gift_list)>0}-->
<!--                    <button type="button" class="layui-btn layui-btn-sm" onclick="$eb.createModalFrame('编辑','{:Url('set_attr')}?id={$news.id}',{w:900,h:660})">设置商品规格</button>-->
<!--                    {/if}-->
                </div>
                <table class="layui-table dis_product_gift  {if $news.gift_type!=2}hide{/if}">
                    <colgroup>
                        <col width="150">
                        <col width="200">
                        <col>
                    ...
                    <tbody id="list-container_gift">
                    {if count($gift_list)>0}
                    {foreach $gift_list as $key=>$vo}
                    <tr>
                        <td><img src="{$vo.image}" style="width: 80px;height: 80px;"></td>
                        <td>{$vo.store_name}</td>
                        <td>{$vo.price}</td>
                        <td>
                            <button type="button" class="layui-btn layui-btn-danger" onclick="delGift(this,{$key})">删除</button>
                        </td>
                    </tr>
                    {/foreach}
                    {else}
...
//    ];
    ...
    function set_type(type){
        ...
        if(type==1){
            ...
            $('.dis_product_gift').addClass('hide');
        }

        if(type==2){
            $('.dis_product_gift').removeClass('hide');
...
//    console.log('goodsList===》',goodsList)
    ...
    var giftList;
    var giftListjson;
    giftListjson=$('#gift_json_input').val();
    if(giftListjson){
        giftList = JSON.parse(giftListjson);
    }else{
        giftList=[];
    }
    ...
    $('.save_news').on('click',function(){
        ...
        for (let index = 0; index < giftList.length; index++) {
            const element = giftList[index];
            giftIdArr.push(element.id)
        }
//        console.log(giftIdArr.toString());
...
//        $news['limit_before_goods_num'] = 0;
        ...
        list.gift_type = $("input[name='gift_type']:checked").val();
...
//        Obj.parentNode.parentNode.parentNode.removeChild(Obj.parentNode.parentNode);
    ...
    function addGift(){
        ...
        var addGiftUrl = '{:Url('select')}?ids=' +giftIdArr.toString()
        $eb.createModalFrame(this.innerText,addGiftUrl,{w:700,h:560})
        interval_id_gift = setInterval('get_gift_cache()',1000);
    }
                ...
                if(re.code == 200){
                    ...
                    for (let index = 0; index < re.data.length; index++) {
                        ...
                        htmllet += `<tr>
                            ...
                            <td>${element.store_name}</td>
                            <td>${element.price}</td>
                            <td><button type="button" class="layui-btn layui-btn-danger" onclick="delGift(this,${index})">删除</button></td>
                        </tr>`;
                    }
                    document.getElementById("list-container_gift").innerHTML = htmllet;
                    //停止轮询
                    clearInterval(interval_id_gift);
                }
            },
