                        ...
                        <div class="layui-card-body">
                            <div class="layui-row layui-col-space10 layui-form-item">
                                ...
                                <div class="layui-col-lg12">
                                    ...
                                    <div class="layui-input-block" v-cloak="">
                                        <button class="layui-btn layui-btn-sm"
                                                :class="{'layui-btn-primary':where.ordertype!=item.value}"
                                                @click="where.ordertype = item.value" type="button"
                                ...
                                <div class="layui-col-lg12">
                                    ...
                                    <div class="layui-input-block" v-cloak="">
                                        <button class="layui-btn layui-btn-sm"
                                                :class="{'layui-btn-primary':where.activity_type!=item.value}"
                                                @click="where.activity_type = item.value" type="button"
                                                v-for="item in activity_type">{{item.name}}
                                            <span v-if="item.count!=undefined" class="layui-badge layui-bg-gray">{{item.count}}</span>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="layui-col-lg12">
                                    <div class="layui-input-block">
                                        ...
                                        <!-- <button @click="excel_import" type="button"
                                                class="layui-btn layui-btn-warm layui-btn-sm export" type="button">
                                            <i class="fa fa-floppy-o" style="margin-right: 3px;"></i>导入快递单并发货
                                        </button> -->
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <!--赠品信息-->
                    <script type="text/html" id="gift_list">
                        {{#  layui.each(d.gift_list, function(index, item){ }}
                        {{#  if(item!=undefined){ }}
                        <div>
                            <span><img style="width: 30px;height: 30px;margin:0;cursor: pointer;"
                                       src="{{item.image}}"></span>
                            <span>{{item.store_name}}</span><span> | ￥{{item.price}}</span>
                        </div>
                        {{# } }}
                        {{#  }); }}
                    </script>
                    ...
                    <script type="text/html" id="act">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            ...
                            {{# if(parseFloat(d.pay_price) >= parseFloat(d.give_store_cash)){ }}
                            <li>
                            <a href="javascript:void(0);"
                        ...
                        <button class="btn btn-primary btn-xs" type="button"
                                onclick="$eb.createModalFrame('发送货','{:Url('order_goods')}?id={{d.id}}',{w:500,h:380})">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                          <!-- <li>
                           ...
                           <li>
                            <a href="javascript:void(0);"
                                   onclick="$eb.createModalFrame('退款审核','{:Url('refund_y')}?id={{d.id}}',{w: 700, h: 500})">

                                    <i class="fa fa-history"></i> 退款审核
                                </a>
                            </li>
                            {{# } }}
...
<script>
    layList.tableList('List', "{:Url('order_list',['real_name'=>$real_name])}", function () {
        return [
            ...
            // {field: 'info', title: '商品信息', templet: "#info", width:'15%', height: 'full-20'},
            {templet:function(d){
                if(d.order_type==1){
                    return '全额付款'
            ...
            }, title: '订单类型', height: 'full-20'},
            {field: 'pay_price', title: '实际支付', width: '8%', align: 'center'},
            {field: 'paid', title: '支付状态', templet: '#paid', width: '7%', align: 'center'},
            {field: 'status', title: '订单状态', templet: '#status', width: '6%', align: 'center'},
            {field: 'mark', title: '用户备注', width: '20%', sort: true, align: 'center'},
    ...
    layList.tool(function (event, data, obj) {
        switch (event) {
            case 'tuikuan':
                $.get("{:url('server_phone')}", data,
                    function (data, textStatus, jqXHR) {
                        if(data.code==200){
                            layer.alert('请联系平台客服:'+data.data.phone, {icon: 5});
                        }
                    },
                    "json"
                );
            ...
            case 'order_info':
                ...
                break;
            case 'order_print':
                $eb.createModalFrame('销售清单', layList.U({a: 'inventory', q: {id: data.id} }),{w: 1360, h: 600});
                break;
        ...
        new Vue({
            ...
            data: {
                ...
                payType: [
                    ...
                    {name: '支付宝支付', value: 2, count: payTypeCount.zhifubao},
                    // {name: '线下支付', value: 3, count: payTypeCount.offline},
                ],
                orderType: [
                    {name: '全部', value: ''},
                    {name: '全款支付', value: 1, count: orderCount.general},
                    {name: '欠款支付', value: 2, count: orderCount.discount},
                    {name: '物流代收', value: 3, count: orderCount.collect},
                ],
                //原筛选备份
               activity_type: [
                   {name: '全部', value: ''},
                   {name: '普通订单', value: 1, count: orderCount.noactivity},
                ...
                orderStatus: [
                    ...
                    {name: '已发货', value: 2, count: orderCount.ds},
                    {name: '已完成', value: 3, count: orderCount.wc},
                    {name: '已取消', value: 4, count: orderCount.qx},
            ...
            watch: {
                ...
                'where.activity_type': function () {
                    this.where.excel = 0;
                    this.getBadge();
                    layList.reload(this.where, true);
                },
                'where.ordertype': function () {
                    this.where.excel = 0;
                    this.getBadge();
                    layList.reload(this.where, true);
                }
