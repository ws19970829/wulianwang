        ...
        <div class="layui-input-block">
            <table class="layui-table">
                ...
                <tr>
                    ...
                    {elseif condition="($key neq 'brokerage') AND ($key neq 'brokerage_two') AND ($key neq 'cost') AND ($key neq 'ot_price') AND ($key neq 'bar_code') AND ($key neq 'weight') AND ($key neq 'volume')"/}
                    {if condition="($key neq 'price') AND ($key neq 'quota')"}
                    <td style="text-align: center"><span>{$item}</span><input type="hidden" 
                    name="attr[{$k}][{$key}]" class="layui-input" value="{$item}"></td>
                    {else/}
                    <td><input type="number" name="attr[{$k}][{$key}]" class="layui-input max" 
                    value="{$item}" min="0" max="{if condition="$key eq 'quota'"}{$vo['stock']}{else/}{$vo['price']}{/if}" oninput="checknum(this)"></td>
                    {/if}{/if}
                    {/volist}
                </tr>
                {/volist}
