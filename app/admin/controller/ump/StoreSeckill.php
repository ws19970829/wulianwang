...
class StoreSeckill extends AuthController
{
    ...
    public function get_seckill_list()
    {
        ...
        if (is_object($seckillList['list'])) $seckillList['list'] = $seckillList['list']->append(['status_text'])->toArray();
        $data = $seckillList['list']['data'];

        foreach ($data as $k => $v) {
            $data[$k]['start_day'] = date('Y-m-d', $data[$k]['start_day']);
            $data[$k]['stop_day'] = date('Y-m-d', $data[$k]['stop_day']);
    ...
    public function create()
    {
        ...
        //     return $menus;
        // })->filterable(1)->col(12);
        $f[] = Form::input('price', '活动价格');
        $f[] = Form::hidden('ot_price', 0);
        $f[] = Form::hidden('cost', 0);
        $f[] = Form::dateRange('section_time', '活动日期');
        $f[] = Form::timeRange('time_range', '秒杀时间');
    ...
    public function save($id = 0)
    {
        ...
        if (count($data['section_time']) < 1) return Json::fail('请选择活动日期');
        if (count($data['time_range']) < 1) return Json::fail('请选择活动时间');
        $data['start_day'] = strtotime($data['section_time'][0] . ' 00:00:00');
        $data['stop_day'] = strtotime($data['section_time'][1] . ' 23:59:59');
        $data['start_time'] = $data['time_range'][0];
        $data['stop_time'] = $data['time_range'][1];
        ...
        $data['ot_price'] = StoreProduct::where('id', $data['product_id'])->value('price');
        if ($id) {
            ...
            if (StoreSeckillModel::checkSeckill($data['product_id'], $data['start_time'], $data['stop_time'], $data['start_day'], $data['stop_day'])) return Json::fail('该商品当前时间段已有秒杀活动');
            $data['add_time'] = time();
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::input('title', '商品名称', $product->getData('title'));
        $f[] = Form::input('price', '活动价格', $product->price);
        ...
        //     return $menus;
        // })->filterable(1)->col(12);
        $f[] = Form::dateRange('section_time', '活动时间', date('Y-m-d H:i:s', (int) $product->getData('start_day')), date('Y-m-d H:i:s', (int) $product->getData('stop_day')));
        $f[] = Form::timeRange('time_range', '秒杀时间', $product->getData('start_time'), $product->getData('stop_time'));
        $f[] = Form::frameImageOne('image', '商品主图片(305*305px)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('500px');
        ...
        $f[] = Form::hidden('cost', $product->getData('cost'));
    ...
    public function attr_list_bak($id)
    {

        if (!$id) return $this->failed('数据不存在!');
        $seckillInfo = StoreSeckillModel::where('id', $id)->find();
        $seckillResult = StoreProductAttrResult::where('product_id', $seckillInfo['product_id'])
            ->where('type', 1)
        ...
        if ($productResult) {
            ...
            $seckillAttr = $this->get_attr($attr, $seckillInfo['product_id'], 1, $id);
            ...
            if ($seckillResult) {
                ...
                $productAttr = $this->get_attr($attr, $seckillInfo['product_id'], 1, $id);
            } else {
                $attr[0]['value'] = '默认';
                $attr[0]['detailValue'] = '';
                $attr[0]['attrHidden'] = '';
    ...
    public function attr_list($id)
    {
        ...
        $productAttr = StoreProductAttrValue::where('type', 0)
            ->where('product_id', $seckillInfo['product_id'])
            ->select();
        $seckillAttr = StoreProductAttrValue::where('type', 1)
            ->where('product_id', $seckillInfo['product_id'])
            ->where('activity_id', $id)
            ->select();
        $productAttr = $productAttr->append(['true_stock'])->toArray();
        $seckillAttr = $seckillAttr->append(['true_stock'])->toArray();
        foreach ($productAttr as $pk => $pv) {
            $productAttr[$pk]['check'] = 0;
            foreach ($seckillAttr as $sv) {
                if ($pv['suk'] == $sv['suk']) {
                    $sv['check'] = 1;
                    $productAttr[$pk] = $sv;
        ...
        $arr = ['suk', 'price', 'true_stock', 'quota', 'moq', 'check'];
        $this->assign('attr', $productAttr);
    ...
    public function save_attr()
    {
        ...
        if (min(array_column($detail, 'quota')) == 0) return Json::fail('限量不能为0');
        $otPrice = $price = min(array_column($detail, 'price'));
        $quota = array_sum(array_column($detail, 'quota'));
        $stock = array_sum(array_column($detail, 'stock'));

        $res = StoreProductAttr::createProductAttr($detail, $productId, 'activity', 1, $data['id']);
        if (!$res) {
    ...
    public function save_attr_bak()
    {
        ...
        StoreProductAttr::createProductAttr($attr, $detail, $productId, 1, $data['id']);
        StoreSeckillModel::where('id', $data['id'])->update(['stock' => $stock, 'quota' => $quota, 'quota_show' => $quota, 'price' => $price, 'ot_price' => $otPrice]);
        return Json::successful('修改成功!');
    }
    ...
    public function set_attr($id)
    {
        ...
        $product_id = db('store_seckill')->where('id', $id)->value('product_id');
        $res = StoreSeckillAttr::createProductAttr($attr, $detail, $product_id, $id);
        if ($res)
    ...
    public function clear_attr($id)
    {
        if (!$id) return $this->failed('商品不存在!');
        $product_id = db('store_seckill')->where('id', $id)->value('product_id');
        if (false !== StoreSeckillAttr::clearProductAttr($product_id, $id) && false !== StoreSeckillAttrResult::clearResult($product_id, $id))
    ...
    public function set_seckill_status($status, $id = 0)
    {
        ...
        $res = StoreSeckillModel::edit(['is_show' => $status], $id);
     ...
     * @return array
     */
    public function get_attr($attr, $id, $type, $activity_id = 0)
    {
        ...
        foreach ($value as $key => $item) {
            ...
            $sukValue = StoreProductAttrValue::where('product_id', $id)
                ->where('type', $type)
                ->where('activity_id', $activity_id)
                ->where('suk', $suk)->column('bar_code,cost,price,ot_price,stock,image as pic,weight,volume,brokerage,brokerage_two,quota', 'suk');
            if (count($sukValue)) {
