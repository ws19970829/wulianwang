...
class StoreCoupon extends AuthController
{
    ...
    public function issue($id)
    {
        ...
        $f[] = Form::radio('is_public', '是否公开显示', 1)->options([['label' => '是', 'value' => 1], ['label' => '否', 'value' => 0]]);
    ...
    public function issue_bag($id)
    {
        if (!CouponModel::be(['id' => $id, 'status' => 1, 'is_del' => 0]))
            return $this->failed('发布的优惠劵已失效或不存在!');
        $f = [];
        $f[] = Form::input('is_bag', '是否是礼包优惠券', '是')->disabled(1);
    ...
    {
        list($_id, $rangeTime, $count, $status, $is_permanent, $full_reduction, $is_give_subscribe, $is_full_give, $is_public, $is_bag) = Util::postMore([
            'id',
            ['range_date', ['', '']],
            ['count', 0],
            ['status', 0],
        ...
        //如果是礼包优惠券，则存1
        if ($is_bag) $is_bag = 1;
        if (StoreCouponIssue::setIssue($id, $count, strtotime($startTime), strtotime($endTime), $count, $status, $is_permanent, $full_reduction, $is_give_subscribe, $is_full_give, $is_public, $is_bag))
