...
class StoreCombination extends AuthController
{
    ...
    public function create()
    {
        ...
        // $f[] = Form::select('temp_id', '拼团运费模板')->setOptions(function () {
        //     $list = ShippingTemplates::getList(['page' => 1, 'limit' => 20]);
        //     $menus = [];
        //     foreach ($list['data'] as $menu) {
        //         $menus[] = ['value' => $menu['id'], 'label' => $menu['name']];
        ...
        // })->filterable(1)->col(12);
        $f[] = Form::dateTimeRange('section_time', '拼团时间');
        // $f[] = Form::number('ot_price', '展示价格')->placeholder('非真实价格')->min(0)->col(20);
        ...
        $f[] = Form::number('goods_num', '成团量')->min(1)->col(16);
    ...
    public function save($id = 0)
    {
        ...
        if ($data['goods_num'] < 1) return Json::fail('请输入正确的成团量');
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::number('goods_num', '成团量', $product->getData('goods_num'))->min(2)->col(12);
    ...
    public function attr_list($id)
    {
        ...
        $productAttr = StoreProductAttrValue::where('type', 0)
            ->where('product_id', $combinationInfo['product_id'])
            ->select();
        $productAttr = $productAttr->append(['true_stock'])->toArray();
        $comAttr = StoreProductAttrValue::where('type', 3)
            ->where('product_id', $combinationInfo['product_id'])
            ->where('activity_id', $id)
            ->select();
        $comAttr = $comAttr->append(['true_stock'])->toArray();
        foreach ($productAttr as $pk => $pv) {
            $productAttr[$pk]['check'] = 0;
            foreach ($comAttr as $sv) {
                if ($pv['suk'] == $sv['suk']) {
                    $sv['check'] = 1;
                    $productAttr[$pk] = $sv;
                }
    ...
    public function attr_list_bak($id)
    {
        if (!$id) return $this->failed('数据不存在!');
        $combinationInfo = StoreCombinationModel::where('id', $id)->find();
        $combinationResult = StoreProductAttrResult::where('product_id', $combinationInfo['product_id'])->where('type', 3)
            ->where('activity_id', $id)
            ->value('result');
        ...
        if ($productResult) {
            ...
            $combinationAttr = $this->get_attr($attr, $combinationInfo['product_id'], 3, $id);
            foreach ($productAttr as $pk => $pv) {
            ...
            if ($combinationResult) {
                ...
                $productAttr = $this->get_attr($attr, $combinationInfo['product_id'], 3, $id);
            } else {
                $attr[0]['value'] = '默认';
                $attr[0]['detailValue'] = '';
                $attr[0]['attrHidden'] = '';
    ...
    public function save_attr()
    {
        ...
        $res = StoreProductAttr::createProductAttr($detail, $productId, 'activity', 3, $data['id']);
        if (!$res) {
            return Json::fail(StoreProductAttr::getErrorInfo());
        }
        StoreCombinationModel::where('id', $data['id'])->update(['stock' => $stock, 'quota' => $quota, 'quota_show' => $quota, 'price' => $price, 'ot_price' => $otPrice]);
    ...
    public function save_attr_bak()
    {
        ...
        StoreProductAttr::createProductAttr($attr, $detail, $productId, 3, $data['id']);
        StoreCombinationModel::where('id', $data['id'])->update(['stock' => $stock, 'quota' => $quota, 'quota_show' => $quota, 'price' => $price, 'ot_price' => $otPrice]);
        return Json::successful('修改成功!');
    }
    ...
    public function set_attr($id)
    {
        ...
        $productId = StoreCombinationModel::where('id', $id)->value('product_id');
        list($attr, $detail) = Util::postMore([
            ['items', []],
            ['attrs', []]
        ], $this->request, true);
        $res = StoreCombinationAttr::createProductAttr($attr, $detail, $productId, $id);
        if ($res)
    ...
    public function clear_attr($id)
    {
        if (!$id) return $this->failed('商品不存在!');
        $productId = StoreCombinationModel::where('id', $id)->value('product_id');
        if (false !== StoreCombinationAttr::clearProductAttr($productId, $id) && false !== StoreCombinationAttrResult::clearResult($productId, $id))
    ...
    public function edit_content($id)
    {
        ...
        $this->assign([
            ...
            'field' => 'description',
            'action' => Url::buildUrl('change_field', ['id' => $productId, 'field' => 'description'])
        ]);
    ...
    public function change_field($id)
    {
        ...
        $productId = StoreCombinationModel::where('id', $id)->value('product_id');
        StoreDescription::saveDescription($data['description'], $productId, 3);
    ...
    public function set_combination_status($status, $id = 0)
    {
        ...
        $productId = StoreCombinationModel::where('id', $id)->value('product_id');
        $res = StoreProductAttrValue::where('product_id', $productId)
            ->where('type', 3)
            ->where('activity_id', $id)
            ->find();
    ...
    public function get_attr($attr, $id, $type, $activity_id = 0)
    {
        ...
        foreach ($value as $key => $item) {
            ...
            $sukValue = StoreProductAttrValue::where('product_id', $id)
                ...
                ->where('suk', $suk)
                ->column('bar_code,cost,price,ot_price,stock,image as pic,weight,volume,brokerage,brokerage_two,quota', 'suk');
            if (count($sukValue)) {
