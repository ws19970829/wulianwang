...
use app\admin\model\system\{
    SystemAttachment,
    ShippingTemplates,
    RoutineCode as RoutineCodeModel
};
...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        $forsale = ProductModel::where('is_del', 0)
            ...
            ->count();
        //仓库中产品
        $warehouse = ProductModel::where('is_del', 0)
            ->where('tenant_id', '=', session('tenant_id'))
            ->where('is_different', 0)
            ->where('product_id', '=', 0)
            ->count();
        //已经售馨产品
        $outofstock = ProductModel::getModelObject(['type' => 4])
            ->where('is_different', 0)
            ->where('tenant_id', '=', session('tenant_id'))
            ->where('product_id', '=', 0)
            ->count();
        //警戒库存
        $policeforce = ProductModel::getModelObject(['type' => 5])
            ->where('is_different', 0)
            ->where('tenant_id', '=', session('tenant_id'))
            ->where('product_id', '=', 0)
            ->count();
        //回收站
        $recycle = ProductModel::where('is_del', 1)
            ->where('tenant_id', '=', session('tenant_id'))
            ->where('is_different', 0)
            ->where('product_id', '=', 0)
            ->count();
    ...
    {
        $list = CategoryModel::getTierList1($id);

        // $menus = [];
        // foreach ($list as $menu) {
        //     $menus[] = ['value' => $menu['id'], 'label' => $menu['html'] . $menu['cate_name'], 'disabled' => $menu['level'] < 3 ? 0 : 1]; //,'disabled'=>$menu['pid']== 0];
        // }
        ...
        if ($id) {
            ...
            $result = StoreProductAttrValue::where('product_id', $id)->where('type', 0)->select();

            $productInfo['items'] = [];
            $productInfo['attrs'] = $result;
            $productInfo['attr'] = ['pic' => '', 'price' => 0, 'cost' => 0, 'ot_price' => 0, 'stock' => 0, 'bar_code' => '', 'weight' => 0, 'volume' => 0, 'brokerage' => 0, 'brokerage_two' => 0, 'discount' => 0, 'discount_gt' => 0, 'discount_lt' => 0, 'moq' => 0, 'suk' => ''];
    ...
    public function save($id)
    {
        ...
        //     if (!isset($item['brokerage_two']) || !$item['brokerage_two']) {
        //         $item['brokerage_two'] = 0;
        //     }
        //     if (($item['brokerage'] + $item['brokerage_two']) > $item['price']) {
        //         return Json::fail('总返佣不能大于商品售价');
        //     }
        // }
        ...
        if ($id) {
            ...
            $attr_res = StoreProductAttr::createProductAttr($detail, $id,'edit');
            if ($attr_res) {
            ...
            $data['uuid'] =  ProductModel::makeUuid();
            ...
            $attr_res = StoreProductAttr::createProductAttr($detail, $res['id']);
            if ($attr_res) {
                ProductModel::commitTrans();
    ...
    public function is_format_attr($id = 0, $type = 0)
    {
        ...
        foreach ($value as $key => $item) {
            ...
            $types = 1;
            if ($id) {
                $sukValue = StoreProductAttrValue::where('product_id', $id)->where('type', 0)->where('suk', $suk)->column('bar_code,cost,price,ot_price,stock,image as pic,weight,volume,brokerage,brokerage_two,discount,discount_gt,discount_lt,advance_sale,advance_day,advance_time,moq,sort', 'suk');
                if (!count($sukValue)) {
                    ...
                    $sukValue[$suk]['moq'] = 0;
                    $sukValue[$suk]['discount'] = 0;
                    $sukValue[$suk]['discount_gt'] = 0;
                    $sukValue[$suk]['discount_lt'] = 0;
                    $sukValue[$suk]['advance_sale'] = 0;
                    $sukValue[$suk]['advance_time'] = 0;
                    $sukValue[$suk]['advance_day'] = date('Y-m-d');
                    $sukValue[$suk]['sort'] = 0;
                }
            } else {
                $sukValue[$suk]['pic'] = '';
            ...
            if ($types) { //编辑商品时，将没有规格的数据不生成默认值
                ...
                $valueNew[$count]['moq'] = $sukValue[$suk]['moq'] ?? 0;
                $valueNew[$count]['discount'] = $sukValue[$suk]['discount'] ?? 0;
                $valueNew[$count]['discount_gt'] = $sukValue[$suk]['discount_gt'] ?? 0;
                $valueNew[$count]['discount_lt'] = $sukValue[$suk]['discount_lt'] ?? 0;
                $valueNew[$count]['advance_sale'] = $sukValue[$suk]['advance_sale'];
                $valueNew[$count]['advance_time'] = $sukValue[$suk]['advance_time'];
                $valueNew[$count]['advance_day'] = $sukValue[$suk]['advance_day'];
                $sukValue[$count]['suk'] =  $sukValue[$suk]['suk'] ?? '';
                $sukValue[$count]['sort'] =  $sukValue[$suk]['sort'] ?? 0;
                $count++;
            }
        }
        $header[] = ['title' => '属性', 'slot' => 'suk', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '库存', 'slot' => 'stock', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '起订量', 'slot' => 'moq', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '优惠量', 'slot' => 'discount', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '大于等于优惠量的售价', 'slot' => 'discount_gt', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '小于优惠量的售价', 'slot' => 'discount_lt', 'align' => 'center', 'minWidth' => 140];
        $header[] = ['title' => '排序', 'slot' => 'sort', 'align' => 'center', 'minWidth' => 140];
     ...
     * @return \think\Response
     */
    public function is_boutique($id)
    {
        if (!$id) return $this->failed('数据不存在');
        if (!ProductModel::be(['id' => $id])) return $this->failed('产品数据不存在');
        if (ProductModel::be(['id' => $id, 'is_boutique' => 0])) {
            $data['is_boutique'] = 1;
            if (!ProductModel::edit($data, $id))
                return Json::fail(ProductModel::getErrorInfo('更改失败，请稍后再试!'));
            else
    ...
    public function is_recommend($id)
    {
        if (!$id) return $this->failed('数据不存在');
        if (!ProductModel::be(['id' => $id])) return $this->failed('产品数据不存在');
        if (ProductModel::be(['id' => $id, 'is_hot' => 0])) {
            $data['is_hot'] = 1;
            if (!ProductModel::edit($data, $id))
                return Json::fail(ProductModel::getErrorInfo('更改失败，请稍后再试!'));
            else
    ...
    {
        $id = $request->param('id');
        $suk = $request->param('suk');

        if ($id != 0) {
            $res = StoreProductAttrValue::where('type', '<>', 0)
                ->where('product_id', $id)
                ->where('is_del',0)
                ->where('suk', $suk)
                ->count();
            if ($res > 0) {
                return Json::fail('该商品有活动开启，无法删除属性');
            } else {
    ...
    public function extension()
    {
        ...
        $img_info = $this->get_product_share_img($product_id);
     ...
     * @return mixed
     */
    function get_product_share_img($product_id)
    {
        //初始化设置
        $tenant_id = ProductModel::where('id', '=', $product_id)->value('tenant_id');
        ...
        $url = $site_url . '/' . 'detail/' . $product_id . '?tenant_id=' . $tenant_id;

        $qCodePath = $this->get_url_qrcode($url, $product_id, 'product_');
        ...
        $product_info = ProductModel::where('id', '=', $product_id)->field('store_name,image')->find();
