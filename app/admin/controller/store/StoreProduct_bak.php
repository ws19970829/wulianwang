...
class StoreProduct extends AuthController
{
    ...
    public function get_product_info($id = 0)
    {
        ...
        if ($id) {
            ...
            if ($productInfo['spec_type'] == 1) {
                ...
                $productInfo['attr'] = ['pic' => '', 'price' => 0, 'cost' => 0, 'ot_price' => 0, 'stock' => 0, 'bar_code' => '', 'weight' => 0, 'volume' => 0, 'brokerage' => 0, 'brokerage_two' => 0, 'discount' => 0, 'discount_gt' => 0, 'discount_lt' => 0, 'moq' => 0];
            } else {
                ...
                $productInfo['attr'] = [
                    ...
                    'discount' => $single['discount'] ?? 0,
                    'discount_gt' => $single['discount_gt'] ?? 0,
                    'discount_lt' => $single['discount_lt'] ?? 0,
                    'advance_sale' => $single['advance_sale'] ?? 0,
                    'advance_day' => $single['advance_day'] ?? date('Y-m-d'),
                    'advance_time' => $single['advance_time'] ?? 0,
                    'moq' => $single['moq'] ?? 0
                ];
            }
    ...
    public function is_format_attr($id = 0, $type = 0)
    {
        ...
        foreach ($value as $key => $item) {
            ...
            $types = 1;
            if ($id) {
                $sukValue = StoreProductAttrValue::where('product_id', $id)->where('type', 0)->where('suk', $suk)->column('bar_code,cost,price,ot_price,stock,image as pic,weight,volume,brokerage,brokerage_two,discount,discount_gt,discount_lt,advance_sale,advance_day,advance_time,moq', 'suk');
                if (!count($sukValue)) {
