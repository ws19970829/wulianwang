...
class SystemConfig extends AuthController
{
    ...
    public function index()
    {
        ...
        $new_list=[];
        if ($type == 3 && $tab_id==9) {//其它分类
            //处理提现设置，隐藏不必要的配置项
            foreach ($list as $key => $value){
                $hidden_arr=[
                    'store_brokerage_statu','store_brokerage_two','extract_time','store_brokerage_price'
                ];
                if(in_array($value['menu_name'],$hidden_arr)){
                    continue;
                }
        ...
        if($type==0 && $tab_id==1){
            foreach ($list as $key => $value){
                $hidden_arr=[
                    'news_slides_limit','cache_config','filing_info'
                    ];
                if(in_array($value['menu_name'],$hidden_arr)){
                    continue;
                }
        ...
        if($type==0 && $tab_id==5){
            foreach ($list as $key => $value){
                $hidden_arr=[
                    'store_user_min_recharge','replenishment_num','extract_time','store_brokerage_price','order_bargain_time','order_seckill_time'
                    ,'order_pink_time','new_order_audio_link','system_delivery_time','offline_pay_status','recharge_switch','new_goods_bananr','recharge_attention'
                ];
                if(in_array($value['menu_name'],$hidden_arr)){
                    continue;
                }
...
//                ConfigModel::edit_by_tenant_id(['value' => json_encode($v)], $k, 'menu_name',session('tenant_id'));
                $where=[
