 ...
 */
class Paygift extends AuthController
{

    protected $type=6;//4限时折扣，5限时秒杀，6支付有礼
    ...
    public function create()
    {
        ...
        if ($id) {
            ...
            //关联的赠品商品
            $gift_list=(new ActivityProductGift())->getProductListByActivityId($id);


            //关联的优惠券
            $coupon_issue_list=(new ActivityCouponIssue())->where('activity_id','=',$id)->select();
...
//        $product_list_json=json_encode($product_list);
        ...
        $this->assign('gift_list_json',$gift_list_json);
        return $this->fetch();
    }
...
//        $product_list_json=json_encode($product_list);
    ...
    public function save(Request $request)
    {
        ...
        if($data['gift_type']==1 && !$data['coupon_ids']){
            return Json::fail('请选择优惠券');
        }

        if($data['gift_type']==2 && !$data['giftIdArr']){
            return Json::fail('请选择赠品');
        }
        ...
        if($data['id']){
            ...
            if($data['gift_type']==1){
                ...
                (new ActivityCouponIssue())->where('activity_id','=',$id)->delete();
                foreach($couponIdsArr as $val){
                    $tem['activity_id']=$id;
                    $tem['coupon_issue_id']=$val;
                    $coupon_data[]=$tem;
                }
                $res=(new ActivityCouponIssue())->saveAll($coupon_data);
                if(!$res){
            ...
            if($data['gift_type']==2){
                $gift_data=[];
                //处理商品和笔记的关联数据-先清空已有数据
                (new ActivityProductGift())->where('activity_id','=',$id)->delete();
                foreach($giftIdArr as $val){
                    $tem['activity_id']=$id;
                    $tem['product_id']=$val;
                    $gift_data[]=$tem;
                }
                $res=(new ActivityProductGift())->saveAll($gift_data);
                if(!$res){
                    BaseModel::rollbackTrans();
                    return Json::fail('保存失败');
                }
