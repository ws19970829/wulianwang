...
use app\admin\model\user\{
    User as UserModel, UserBill as UserBillAdmin, UserLevel, UserGroup, UserTagLog, UserTaskFinish
};
...
class User extends AuthController
{
    ...
    public function index()
    {
        ...
        //用户tag标签
        $list=\app\admin\model\user\UserTag::getSytemListToSelect([]);
...
//        $f[] = Form::number('integration', '积分')->min(0);
    ...
    public function get_user_list()
    {
        ...
        $return=UserModel::getUserList($where);
        $user_list=collect($return['data']);
        if($return['count']>0){
            foreach($user_list as $key=>$val){
                //获取用户标签
                $user_list[$key]['tags']=UserTagLog::getTagsByUID($val['uid'],'string');

            }
        }
        $return['data']=$user_list;
        return Json::successlayui($return);
    }
    ...
    public function grant_tags($id)
    {
        ...
        $list=\app\admin\model\user\UserTag::getSytemListToSelect($where);
        $this->assign('list',$list);
//        $this->assign(\app\admin\model\user\UserTag::getSytemListToSelect($where));
        return $this->fetch();
    }
     ...
     * @return string
     */
    public function user_tags_list(){

        $uid=input('param.uid');
        $this->assign('uid',$uid);
        $list=UserTagLog::getTagsByUID($uid);
        $this->assign('list',$list);
        return $this->fetch();
//        $this->assign(\app\admin\model\user\UserTag::getSytemListToSelect($where));
    }
     ...
     * 删除用户标签
     */
    public function del_user_tags(){
        $tag_id=input('param.id');
        $uid=input('param.uid');
        $res=UserTagLog::where('uid','=',$uid)->where('tag_id','=',$tag_id)->delete();
        if($res){
            return Json::successful('删除成功');
        }else{
            return Json::fail('删除失败，请稍后再试');
    ...
    public function save_add_tags(){
        ...
        foreach($uid_arr as $val){
            ...
            $res=UserTagLog::where($data)->value('id');
            if(!$res){
                UserTagLog::create($data);
            }
    ...
    {
//        dump(UserModel::getUserDetailed($uid));exit;
        $this->assign([
            'uid' => $uid,
