 ...
 */
class Business extends AuthController
{
    /**
     * 显示资源列表
     *
    ...
    public function business_list()
    {
        ...
        return Json::successlayui(BusinessModel::getBusinessList($where));
    }
...
//        $news['author'] = '';
        ...
        $product_list = [];
        if ($id) {
            $news = BusinessModel::where('id', $id)->find();
...
//        $product_list_json=json_encode($product_list);
    ...
    public function save(Request $request)
    {
        ...
        if($data['id']){
            $id=$data['id'];
            unset($data['id']);
            BusinessModel::edit($data,$id,'id');
            //处理商品和合作商家的关联数据-先清空已有数据
            (new BusinessProduct())->where('business_id','=',$id)->delete();
            $data=[];
            ...
            (new BusinessProduct())->saveAll($data);
        }else{
            unset($data['id']);
            $res=BusinessModel::create($data);
            if($res){
                $business_id=$res->getData('id');
                //处理商品和合作商家的关联数据
                $data=[];
                foreach($goodsIdArr as $val){
                    $tem['business_id']=$business_id;
                    $tem['product_id']=$val;
                    $data[]=$tem;
                }
                (new BusinessProduct())->saveAll($data);
...
//                $list = (sort_list_tier((CategoryModel::where('id','<>',$id)->select()->toArray(),'顶级','pid','cate_name'));
    ...
    {
        if (!BusinessModel::where('id','=',$id)->update(['is_del'=>1]))
            return Json::fail(BusinessModel::getErrorInfo('删除失败,请稍候再试!'));
        else
    ...
    public function set_show($is_show = '', $id = '')
    {
        ...
        if (\app\admin\model\business\Business::setShow($id, (int)$is_show)) {
