...
class WechatUser extends BaseModel
{
    ...
    public static function agentSystemPage($where = [])
    {
        ...
        $broken_time = intval((new PublicController())->getSysConfigValue('extract_time',$tenant_id));
        $search_time = time() - 86400 * $broken_time;
        foreach ($data as &$item) {
            if ($spread_uid = User::where('uid', $item['uid'])->where('tenant_id','=',$tenant_id)->value('spread_uid')) {
                if ($user = User::where('uid', $spread_uid)->where('tenant_id','=',$tenant_id)->field(['uid', 'nickname'])->find()) {
                    $item['spread_name'] = $user['nickname'] . '/' . $user['uid'];
                }
            }
            $item['spread_count'] = User::where('spread_uid', $item['uid'])->where('tenant_id','=',$tenant_id)->count();
            //返佣 +
    ...
    * */
    public static function getStairOrderList($where)
    {
        ...
        $data = self::setSairOrderWhere($where, new StoreOrder())
            ...
            ->select();
        $data = count($data) ? $data->toArray() : [];
        $Info = User::where('uid', $where['uid'])->where('tenant_id','=',$tenant_id)->find();
        foreach ($data as &$item) {
            $userInfo = User::where('uid', $item['uid'])->where('tenant_id','=',$tenant_id)->find();
            $item['user_info'] = '';
            $item['avatar'] = '';
            if ($userInfo) {
            ...
            $item['number_price'] = UserBill::where(['uid'=>$Info['uid'], 'category' => 'now_money', 'type' => 'brokerage', 'link_id' => $item['id']])->where('tenant_id','=',$tenant_id)->value('number');
    ...
    * */
    public static function getSpreadBadge($where)
    {
        $tenant_id=session('tenant_id');
        $where['is_time'] = 1;
        $listuids = self::setSpreadWhere($where)
            ->where('u.tenant_id','=',$tenant_id)
            ->field('u.uid')
            ->select();
        $newUids = [];
        ...
        if ($data['sum_count']) {
            //发展会员人数
            $data['spread_sum'] = User::where('spread_uid', 'in', $uids)
                ->where('tenant_id','=',$tenant_id)
                ->count();
            //订单总数
            $data['order_count'] = StoreOrder::where('uid', 'in', $uids)
                ->where('tenant_id','=',$tenant_id)
                ->where(['paid' => 1, 'refund_status' => 0])->count();
            //订单金额
            $data['pay_price'] = StoreOrder::where('uid', 'in', $uids)
                ->where('tenant_id','=',$tenant_id)
                ->where(['paid' => 1, 'refund_status' => 0])->sum('pay_price');
            //可提现金额
            $data['number'] = User::where('uid', 'in', $uids)
                ->where('tenant_id','=',$tenant_id)
                ->sum('brokerage_price');
            //提现次数
