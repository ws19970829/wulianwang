...
class SystemAttachment extends BaseModel
{
    ...
    public static function attachmentAdd($name, $att_size, $att_type, $att_dir, $satt_dir = '', $pid = 0, $imageType = 1, $time = 0, $module_type = 1)
    {
        ...
        $data['pid'] = $pid;
        $tenant_id=session('tenant_id')?session('tenant_id'):0;
    ...
    public static function getImageList($where)
    {
        ...
        $count = $where['pid'] ?
            self::where(['pid' => $where['pid'], 'module_type' => 1])->where('tenant_id',session('tenant_id'))->count()
            : self::where('module_type', 1)->where('tenant_id',session('tenant_id'))->count();
