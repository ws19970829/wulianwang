...
class SystemAdmin extends BaseModel
{
    ...
    public static function login($account, $pwd)
    {
        ...
        //商家后台判断，如果不是平台方，而且tenant_id为0，则不让登录
        if($adminInfo['roles']!=2 && !$adminInfo['tenant_id'])  return self::setErrorInfo('该账号权限不符，无法从本后台登录');
    ...
    public static function setLoginInfo($adminInfo)
    {
        ...
        //如果角色是平台方
        if($adminInfo['roles']==2){
            $tenant_id=$adminInfo['id'];
        }else{
            $tenant_id=$adminInfo['tenant_id'];
        }
