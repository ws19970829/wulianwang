...
class SystemGroupData extends BaseModel
{
    ...
    public static function getList($params)
    {
        ...
        if ($params['tenant_id'] !== '') $model = $model->where('tenant_id', $params['tenant_id']);
        $model = $model->order('sort desc,id ASC');
        $res = self::page($model, function ($item, $key) {
    ...
    public static function getGroupDataByTenant($config_name, $limit = 0, $tenant_id = 0)
    {
        $group = SystemGroup::where('config_name', $config_name)
            ->where('tenant_id', $tenant_id)
            ->field('id,name,info,config_name')
            ->find();
        if (!$group) return false;
        $group['data'] = self::getAllValue1($group['id'], $limit, $tenant_id);
        return $group;
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function getAllValue1($id, $limit = 0)
    {
        $model = self::alias('a')->field('a.*,b.config_name')->join('system_group b', 'a.gid = b.id')
            ->where("b.id", $id)->where("a.status", 1)
