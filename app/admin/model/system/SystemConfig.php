...
class SystemConfig extends BaseModel
{
    ...
    public static function setValue($menu, $value)
    {
        ...
        $bool = self::where('tenant_id', '=', session('tenant_id'))->edit(['value' => json_encode($value)], $menu, 'menu_name');
        return $bool;
    }
     ...
     */
    public static function getConfigValue($menu, $tenant_id = false)
    {
        $condition = [];
        if ($tenant_id !== false) {
            $condition[] = ['tenant_id', '=', $tenant_id];
        }
        if (empty($menu) || !($config_one = self::where('menu_name', $menu)->where($condition)->find())) return false;
     ...
     * @return array
     */
    public static function getMore($menus, $tenant_id = 0)
    {
        ...
        $list = self::where('menu_name', 'IN', $menus)
     ...
     * @return array
     */
    public static function getAllConfig($tenant_id = 0)
    {
        $tenant_id = $tenant_id ? $tenant_id : session('tenant_id');
        $tenant_id = $tenant_id ? $tenant_id : 0;
        $list = self::where('tenant_id', '=', $tenant_id)->column('value', 'menu_name') ?: [];
        foreach ($list as $menu => $value) {
            $list[$menu] = json_decode($value, true);
        }
        return $list;
