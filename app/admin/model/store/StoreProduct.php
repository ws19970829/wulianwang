...
class StoreProduct extends BaseModel
{
    ...
    {
        do {
            $uuid = '800' . mt_rand(10000, 99999);
            $res = self::where('uuid', $uuid)->count();
        } while ($res);
        return $uuid;
    }
    ...
    public static function getModelObject($where = [])
    {
        ...
        $model = $model->where('is_different', 0);        //非异业商品
        if (!empty($where)) {
            $type = $where['type'] ?? 0;
            switch ((int) $type) {
                case 1:
     ...
     * @return object
     */
    public static function getDifferentModelObject($where = [])
    {
        $model = new self();
        $model = $model->where('tenant_id', '=', session('tenant_id'))
    ...
    public static function ProductList($where)
    {
        ...
        if (isset($where['is_daixiao']) && $where['is_daixiao'] == 1) {
            $model = $model->where('product_id', '>', 0);
        }

        if (isset($where['is_daixiao']) && $where['is_daixiao'] == 0) {
            $model = $model->where('product_id', '=', 0);
        }
    ...
    public static function ProductListToProductLog($where)
    {
        ...
        if ($where['excel'] == 1) {
            ...
            PHPExcelService::setExcelHeader(['产品名称', '价格', '曝光次数', '曝光人数(UV)', '访问数', '访客数(IP)', '浏览量(PV)', '加购人数', '加购件数', '支付人数', '单品转换率'])
    ...
    {
        $model = self::getDifferentModelObject($where);
        if ($where['excel'] == 0) $model = $model->page((int) $where['page'], (int) $where['limit']);
        $data = ($data = $model->select()) && count($data) ? $data->toArray() : [];
        ...
        $count = self::getDifferentModelObject($where)->count();
     ...
     * @return array
     */
    public static function ProductListBySelect($where)
    {
        $model = self::getModelObject($where);
        $data = ($data = $model->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item) {
