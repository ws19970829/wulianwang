...
class UserExtract extends BaseModel
{
    ...
    public static function extractStatistics()
    {
        ...
        $data = [];
        //待提现金额
        $data['withdraw_allow'] = db('store_order')
            ->where('is_del', 0)
            ->where('status', 2)
            ...
            ->sum('give_store_cash');
        //不可提现金额
        $data['withdraw_disable'] = db('store_order')->where([
            'tenant_id' => $tenant_id,
            'refund_status' => 0,
            'is_del' => 0
        ])->where('status', '<>', 2)
            ->whereNotIn('withdraw', '0,1,2')
            ->sum('give_store_cash');

        //申请提现的金额
        $data['withdraw_review'] = db('store_order')->where([
            'tenant_id' => $tenant_id,
            'refund_status' => 0,
            'is_del' => 0
        ])->whereIn('withdraw', '0,2')
            ->sum('give_store_cash');
    ...
    public static function getExtractHead()
    {
        $tenant_id = session('tenant_id');
        //本月提现人数
        $month = self::getModelTime(['data' => 'month'], self::where('status', 1))
            ->where('tenant_id', '=', $tenant_id)
            ->group('uid')
            ->count();
        //本月提现笔数
        $new_month = self::getModelTime(['data' => 'month'], self::where('status', 1))
            ->where('tenant_id', '=', $tenant_id)
            ->distinct(true)
            ->count();
        //上月提现人数
        $last_month = self::whereTime('add_time', 'last month')
            ->where('tenant_id', '=', $tenant_id)
            ->where('status', 1)
            ->group('uid')
            ->distinct(true)
            ->count();
        //上月提现笔数
        $last_count = self::whereTime('add_time', 'last month')
            ->where('tenant_id', '=', $tenant_id)
            ->where('status', 1)
            ->count();
        //本月提现金额
        $extract_price = self::getModelTime(['data' => 'month'], self::where('status', 1))
            ->where('tenant_id', '=', $tenant_id)
            ->sum('extract_price');
        //上月提现金额
        $last_extract_price = self::whereTime('add_time', 'last month')
            ->where('tenant_id', '=', $tenant_id)
            ->where('status', 1)
            ->sum('extract_price');
        ...
        return [
            [
                'name' => '总提现人数',
                'field' => '个',
                'count' => self::where('status', 1)->where('tenant_id', '=', $tenant_id)->group('uid')->count(),
                'content' => '',
            ...
            [
                'name' => '总提现笔数',
                'field' => '笔',
                'count' => self::where('status', 1)->where('tenant_id', '=', $tenant_id)->distinct(true)->count(),
                'content' => '',
            ...
            [
                ...
                'content' => '提现总金额',
                'background_color' => 'layui-bg-cyan',
                'sum' => self::where('status', 1)->where('tenant_id', '=', $tenant_id)->sum('extract_price'),
                'class' => 'fa fa-line-chart',
            ],
            [
                'name' => '上月提现人数',
