...
use app\models\store\StoreOrder as StoreStoreOrder;
...
class User extends BaseModel
{
    ...
    protected function getArreasAttr($v, $data)
    {
        $total = 0;
        $list = StoreOrder::where('uid', $data['uid'])
            ...
            ->toArray();
        foreach ($list as $v) {
            $price = bcsub($v['total_price'], $v['coupon_price'], 2);
            $total = bcadd($total, bcsub($price, $v['business_rate'], 2), 2);
        }

        $arrears = UserArrears::where('uid', $data['uid'])->where('is_del', 0)->sum('money');
        return bcsub($total, $arrears, 2);
    }
    ...
    public static function setWhere($where)
    {
        ...
        // $model=$model->where('u.tenant_id','=',session('tenant_id'));
        ...
        if ($where['change_user_time'] != '') {
            list($startTime, $endTime) = explode(' - ', $where['change_user_time']);
            $model = $model->where('u.last_time', '>', strtotime($startTime));
            $model = $model->where('u.last_time', '<', strtotime($endTime) + 24 * 3600);
        }
        if ($where['price_num_min'] != '') {
            $model = $model->where('u.pay_count', '>=', $where['price_num_min']);
        }
        if ($where['price_num_min'] != '') {
            $model = $model->where('u.pay_count', '<=', $where['price_num_max']);
        }
        if ($where['price_num_min'] != '' && $where['price_num_max'] != "") {
            $model = $model->where('u.pay_count', '<=', $where['price_num_max']);
            $model = $model->where('u.pay_count', '>=', $where['price_num_min']);
        }
        if ($where['user_time'] != '') {
        ...
        if ($where['tags'] !== '') {
            $uid_arr = UserTagLog::where('tag_id', '=', $where['tags'])->field('uid')->select();
            $uid_arr = $uid_arr->column('uid');
            $model = $model->where('u.uid', 'in', $uid_arr);
        }
        return $model;
    }
    ...
    public static function getUserList($where)
    {
        ...
        if (!empty($where['arrears'])) {
            $uid = StoreOrder::where('is_del', 0)->where('status', 'in', '1,2')
                ->where('paid', 1)
