...
class StoreSeckillAttr extends BaseModel
{
    /**
     * 模型名称
     * @var string
     */
    protected $name = 'store_seckill_attr';
    ...
    public static function createProductAttr($attrList, $valueList, $productId, $activity_id)
    {
        ...
        $res = false !== $attrModel->saveAll($attrGroup)
            ...
            && false !== StoreSeckillAttrResult::setResult($result, $productId, $activity_id);
        self::checkTrans($res);
        if ($res)
            return true;
        else
    ...
    public static function clearProductAttr($productId, $activity_id)
    {
        ...
        $res = false !== self::where('product_id', $productId)
            ...
            ->delete()
            && false !== StoreSeckillAttrValue::clearProductAttrValue($productId, $activity_id);
        if (!$res)
