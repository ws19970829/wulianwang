...
class StoreSeckill extends BaseModel
{
     ...
     */
    public static function checkSeckill($product_id, $start_time, $end_time, $start_day, $end_day)
    {

        return self::where('product_id', $product_id)
            ->whereTime('start_time', '<=', $start_time)
            ->whereTime('stop_time', '>', $end_time)
            ->whereTime('start_day', '<=', $start_day)
            ->whereTime('stop_day', '>=', $end_day)
            ->where('is_del', 0)
            ->find();
    }
