...
class StoreCombinationAttr extends BaseModel
{
    /**
     * 模型名称
     * @var string
     */
    protected $name = 'store_combination_attr';
    ...
    public static function createProductAttr($attrList, $valueList, $productId, $activity_id)
    {
        ...
        self::beginTrans();
        if (!self::clearProductAttr($productId, $activity_id)) return false;
        $res = false !== $attrModel->saveAll($attrGroup)
            && false !== $attrValueModel->saveAll($valueGroup)
            && false !== StoreCombinationAttrResult::setResult($result, $productId,$activity_id);
        self::checkTrans($res);
        if ($res)
            return true;
        else
    ...
    public static function clearProductAttr($productId, $activity_id)
    {
        ...
        $res = false !== self::where('product_id', $productId)
            ...
            ->delete()
            && false !== StoreCombinationAttrValue::clearProductAttrValue($productId, $activity_id);
        if (!$res)
