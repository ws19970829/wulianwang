...
class StoreCombination extends BaseModel
{
    ...
    public static function setWhere($where, $model = null)
    {
        ...
        $model = $model->where('c.tenant_id', session('tenant_id'));
    ...
    public static function systemPage($where)
    {
        ...
        return self::page($model, null, $where, $where['limit']);
    }
    ...
    {
        return self::where('is_del', 0)->where('tenant_id',session('tenant_id'))->count();
    }
    ...
    public static function getCombinationHost($limit = 0)
    {
        ...
        $model = $model->alias('c');
        $model = $model->join('StoreProduct s', 's.id=c.product_id');
        $model = $model->field('c.id,c.image,c.price,c.sales,c.title,c.goods_num,s.price as product_price');
        $model = $model->where('c.is_del', 0);
    ...
    public static function getbadge($where, $type)
    {
        ...
        $replenishment_num = (int) sys_config('replenishment_num');
     ...
     * @return mixed
     */
    public static function getBargainRefundList($where = [])
    {
     ...
     */
    public function getIsValidAttr($v, $data)
    {
        if (time() > strtotime($data['stop_time'])) return 1;
        return 0;
    }
