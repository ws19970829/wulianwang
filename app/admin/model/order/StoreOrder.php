...
class StoreOrder extends BaseModel
{
    ...
    public function getArreasAttr($v, $data)
    {
        ...
        $price = bcadd($data['pay_price'], $data['pay_postage']);
        return bcsub($price, $data['business_rate']);
    }

    public function getOrderTypeTextAttr($v, $data)
    {
    ...
    {
        return db('express')->where('id', $data['express_id'])->value('name') ?? '无';
    }

    /**
     * @Author  lingyun
     * @Desc
     * @param int $ordertype    1-普通订单，2--异业订单，3-代销订单
     * return mixed
     */
    public static function orderCount($orderType = '')
    {
        $tenant_id = session('tenant_id');
        $map = 'tenant_id = ' . $tenant_id;
        // $data['ys'] = self::statusByWhere(9, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['wz'] = self::statusByWhere(0, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['wf'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'shipping_type' => 1, 'status' => 0, 'paid' => 1, 'refund_status' => 0])
            ->where($map)->count();
        $data['ds'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'shipping_type' => 1, 'status' => 1, 'paid' => 1, 'refund_status' => 0])
            ->where($map)->count();
        $data['wc'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'shipping_type' => 1, 'status' => 2, 'paid' => 1])
            ->where($map)->count();
        // $data['dp'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'shipping_type' => 1, 'status' => 2, 'paid' => 1]);
        $data['qx'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'status' => 4])
            ->where('refund_status', 0)
            ->where($map)
            ->count();
        // $data['jy'] = self::statusByWhere(4, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['tk'] = self::statusByWhere(-1, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['yt'] = self::statusByWhere(-2, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['del'] = self::statusByWhere(-4, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['write_off'] = self::statusByWhere(5, new self())->where(['is_system_del' => 0])->where($map)->count();
        $data['general'] = self::where(['order_type' => 1, 'is_del' => 0])
            ->where($map)
            ->count();
        $data['noactivity'] = self::where('seckill_id', '')
            ->where('combination_id', '')
            ->where('is_del', 0)
            ->where($map)
            ->count();
        $data['pink'] = self::where('combination_id', '<>', '')
            ->where('refund_status', 0)
        ...
        $data['bargain'] = self::where('bargain_id', '>', 0)
            ...
            ->count();
        //欠款支付
        $data['discount'] = self::where(['order_type' => 2, 'is_del' => 0])
            ->where($map)
            ->count();
        $data['collect'] = self::where(['order_type' => 3, 'is_del' => 0])
            ->where($map)
            ->count();
        $data['seckill'] = self::where('seckill_id', '<>', '')
            ->where('is_del', 0)
            ->where('refund_status', 0)
            ->where('is_system_del', 0)
            ->where($map)
            ->count();
        $data['gift'] = self::where('activity_type', '=', 6)
            //            ->where('tenant_id','=',$tenant_id)
            ->where('is_system_del', 0)
            ->where($map)
            ->count();
    ...
    public static function OrderList($where)
    {
        $model = self::getOrderWhere($where, self::alias('a')
            ...
            ->field('a.*,r.nickname,r.account,r.spread_uid');
        if ($where['order'] != '') {
        ...
        if (isset($where['excel']) && $where['excel'] == 1) {
            ...
            // $data = $model->page((int) $where['page'], (int) $where['limit'])->select()->toArray();
            $sql = $model->page((int) $where['page'], (int) $where['limit'])->fetchSql(true)->select();
        }

        $data = FacadeDb::query($sql);
        ...
        foreach ($data as &$item) {
            ...
            $item['cur_tenant_id'] = session('tenant_id');      //当权登录用户
            $item['_info'] = $_info;
            ...
            if ($item['status'] == 4 && $item['refund_status'] == 0) {
                ...
                if ($item['plat_verify']) {
                    ...
                    <b style="color:#f124c7">申请退款：平台审核中</b><br/>
HTML;
                } else {
                    $item['status_name'] = <<<HTML
                    <b style="color:#f124c7">申请退款：等待审核</b><br/>
HTML;
        ...
        $count = self::getOrderWhere(
            $where,
            self::alias('a')->join('user r', 'r.uid=a.uid', 'LEFT'),
            'a.',
            'r'
        )
    ...
    public static function statusByWhere($status, $model = null, $alert = '')
    {
        ...
        if ('' === $status)
            ...
            return $model->where($alert . 'paid', 1)->where($alert . 'refund_status', 2)->where($alert . 'status', 4);
        else if ($status == -3) //退款
            return $model->where($alert . 'paid', 1)->where($alert . 'refund_status', 'in', '1,2')->where($alert . 'is_del', 0);
        else if ($status == -4) //已删除
            return $model->where($alert . 'is_del', 1)->whereOr($alert . 'is_system_del', 1);
    ...
    public static function getOrderWhere($where, $model, $aler = '', $join = '')
    {
        ...
        if (!empty($where['ordertype'])) {
            $model =  $model->where('order_type', $where['ordertype']);
        }

        if (isset($where['status'])) {
            switch ($where['status']) {
                case '0':
                    $model = $model->where('a.status', 0)->where('paid', 0)->where('is_del', 0);
                    break;
                case '1':
                    $model =  $model->where('a.status', 0)->where('paid', 1)->where('is_del', 0)->where('refund_status', 0);
                    break;
                case '2':
                    $model =  $model->where('a.status', 1)->where('paid', 1)->where('is_del', 0)->where('refund_status', 0);
                    break;
                case '3':
                    $model =  $model->where('a.status', 2);
                    break;
                case '4':
                    $model =  $model->where('a.status', 4)->where('refund_status', 0);
                    break;
                case '-1':
                    $model = $model->where('refund_status', 1);
                    break;
                case '-2':
                    $model = $model->where('a.status', 4)->where('refund_status', 2);
                    break;
                case '-4':
                    $model = $model->where('is_del', 1)->whereOr('is_system_del', 1);
                    break;
            }
        } else {
            $model = $model->where('is_system_del', 0)->where('is_del', 0);
        }
        if (!empty($where['activity_type'])) {
            switch ($where['activity_type']) {
                case '1':
                    $model = $model->where('a.seckill_id', '')->where('a.combination_id', '');
                    break;
                case '2':
                    $model = $model->where([['a.combination_id', '>', 0]])
                    ->where('refund_status', 0)
                    ->where('is_del',0)
                    ->where('is_system_del',0);
                    break;
                case '3':
                    $model = $model->where('a.seckill_id', '<>', '')
                    ->where('refund_status', 0)
                    ->where('is_del',0)
                    ->where('is_system_del',0);
                    break;
        ...
        if (isset($where['pay_type'])) {
            switch ($where['pay_type']) {
                case 1:
                    ...
                    break;
                case 2:
                    $model = $model->where($aler . 'pay_type', 'zhifubao');
                    break;
                case 3:
            ...
            switch ($where['type']) {
                ...
                case 1:
                    $model = $model->where($aler . 'combination_id', 0)
                        ->where($aler . 'seckill_id', 0)
                        ->where($aler . 'bargain_id', 0)
                        ->where($aler . 'activity_id', 0);
                    break;
                case 2:
                    ...
                    $model = $model
                        ->where($aler . 'activity_id', '>', 0)
                        ->where($aler . 'activity_type', "=", 4);
                    break;
                case 3:
                    $model = $model
                        ->where($aler . 'activity_id', '>', 0)
                        ->where($aler . 'activity_type', "=", 5);
                    break;
                case 4:
                    $model = $model
                        ->where($aler . 'activity_id', '>', 0)
                        ->where($aler . 'activity_type', "=", 6);
                    break;
            }
        }
     ...
     * @return mixed
     */
    public static function getOrderWhere_bak($where, $model, $aler = '', $join = '')
    {
        //        $model = $model->where('combination_id',0);
        $model = $model->where('is_system_del', 0);
        if (isset($where['status']) && $where['status'] != '') {
    ...
    public static function getOrderPrice($where)
    {
        ...
        //平台方拆分处理
        //        if(isset($where['ordertype']) && $where['ordertype'] != 3){
        //            $whereData['tenant_id']=session('tenant_id');
        //        }else{
        //            $model->where('(tenant_id='.session('tenant_id').') or (org_tenant_id='.session('tenant_id').')')->where('is_consignment',1);
        //        }
        if (isset($where['ordertype'])) {
            if ($where['ordertype'] == 1) {       //普通订单
                $whereData['tenant_id'] = session('tenant_id');
            } else if ($where['ordertype'] == 2) {     //异业订单
                $model->where('org_tenant_id=' . session('tenant_id'))->where('is_consignment', 0);
            } else {      //代销订单
                $model->where('(tenant_id=' . session('tenant_id') . ') or (org_tenant_id=' . session('tenant_id') . ')')->where('is_consignment', 1);
            }
        } else {
        ...
        if (isset($where['ordertype'])) {
            if ($where['ordertype'] == 1) {       //普通订单
                $ids = self::getOrderWhere($where, $model)
                    ->where($whereData)
                ...
                $ids = self::getOrderWhere($where, $model)
                    ...
                    ->where('org_tenant_id=' . session('tenant_id'))
                    ->column('id');
            } else {      //代销订单
                $ids = self::getOrderWhere($where, $model)
                    ->where($whereData)
                    ->where('is_consignment', '=', 1)
                    ->where('(tenant_id=' . session('tenant_id') . ') or (org_tenant_id=' . session('tenant_id') . ')')
                    ->column('id');
            }
        } else {
            $ids = self::getOrderWhere($where, $model)
        ...
        //        if(isset($where['ordertype']) && $where['ordertype'] != 3) {
        //            $price['refund_price'] = self::getOrderWhere($where, $model)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->where('tenant_id',session('tenant_id'))->sum('refund_price');
        //        }else{
        //            $price['refund_price'] = self::getOrderWhere($where, $model)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->where('is_consignment',1)->where('(tenant_id='.session('tenant_id').') or (org_tenant_id='.session('tenant_id').')')->sum('refund_price');
        //        }

        if (isset($where['ordertype'])) {
            if ($where['ordertype'] == 1) {
                $price['refund_price'] = self::getOrderWhere($where, $model)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->where('tenant_id', session('tenant_id'))->sum('refund_price');
            } else if ($where['ordertype'] == 2) {
                $price['refund_price'] = self::getOrderWhere($where, $model)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->where('is_consignment', 1)->where('org_tenant_id=' . session('tenant_id'))->sum('refund_price');
            } else {
                $price['refund_price'] = self::getOrderWhere($where, $model)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->where('is_consignment', 1)->where('(tenant_id=' . session('tenant_id') . ') or (org_tenant_id=' . session('tenant_id') . ')')->sum('refund_price');
            }
        } else {
        ...
        if (isset($where['ordertype'])) {
            if ($where['ordertype'] == 1) {       //普通订单
                $sumNumber = self::getOrderWhere($where, $model)->where($whereData)->where('tenant_id', session('tenant_id'))->field([
                ...
                $sumNumber = self::getOrderWhere($where, $model)->where($whereData)->where('is_consignment', 0)->where('org_tenant_id=' . session('tenant_id'))->field([
                    'sum(total_num) as sum_total_num',
                    'count(id) as count_sum',
                    'sum(pay_price) as sum_pay_price',
                    'sum(pay_postage) as sum_pay_postage',
                ...
                $sumNumber = self::getOrderWhere($where, $model)->where($whereData)->where('is_consignment', 1)->where('(tenant_id=' . session('tenant_id') . ') or (org_tenant_id=' . session('tenant_id') . ')')->field([
                    'sum(total_num) as sum_total_num',
                    'count(id) as count_sum',
                    'sum(pay_price) as sum_pay_price',
                    'sum(pay_postage) as sum_pay_postage',
        ...
        //        if(isset($where['ordertype']) && $where['ordertype'] != 3) {
        //            $list = self::getOrderWhere($where, $model)->where($whereData)->where('tenant_id',session('tenant_id'))->group('pay_type')->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
        //        }else{
        //            $list = self::getOrderWhere($where, $model)->where($whereData)->where('is_consignment',1)->where('(tenant_id='.session('tenant_id').') or (org_tenant_id='.session('tenant_id').')')->group('pay_type')->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
        //        }
        ...
        if (isset($where['ordertype'])) {
            if ($where['ordertype'] == 1) {   //普通订单
                ...
                $list = self::getOrderWhere($where, $model)->where($whereData)->where('is_consignment', 0)->where('org_tenant_id=' . session('tenant_id'))->group('pay_type')->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
            } else {      //代销订单
                $list = self::getOrderWhere($where, $model)->where($whereData)->where('is_consignment', 1)->where('(tenant_id=' . session('tenant_id') . ') or (org_tenant_id=' . session('tenant_id') . ')')->group('pay_type')->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
            }
        } else {
     ...
     * @return array
     */
    public static function payTypeCount($orderType = 1)
    {
        $tenant_id = session('tenant_id');
        $where['real_name'] = '';
        $where['data'] = '';
        ...
        $where['order'] = '';
        $where['pay_type'] = 1;
        $weixin = self::getOrderWhere($where, new self)->where('tenant_id', $tenant_id)->count();
        $where['pay_type'] = 2;
        $zhifubao = self::getOrderWhere($where, new self)->where('tenant_id', $tenant_id)->count();
        $where['pay_type'] = 3;
        $offline = self::getOrderWhere($where, new self)->where('tenant_id', $tenant_id)->count();
        return compact('weixin', 'zhifubao', 'offline');
    }

    public function getShopNameAttr($v, $data)
    {
        return db('system_admin')->where('id', $data['tenant_id'])->value('real_name') ?? '';
    }
}
