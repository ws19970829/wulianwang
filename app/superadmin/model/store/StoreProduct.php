...
namespace app\superadmin\model\store;
...
use app\superadmin\model\store\StoreCategory as CategoryModel;
use app\superadmin\model\ump\{
    StoreBargain,
    StoreCombination,
    StoreSeckill
};
...
class StoreProduct extends BaseModel
{
    ...
    public static function getModelObject($where = [])
    {
        ...
        if (!empty($where['shop_id'])) {
            $model = $model->where('tenant_id', $where['shop_id']);
        } else {
        ...
        if (!empty($where)) {
            ...
            switch ((int) $type) {
                ...
                case 5:
                    ...
                    $model = $model->where(['is_show' => 1, 'is_del' => 0])->where('stock', '<=', $store_stock);
                    break;
                case 6:
    ...
    public static function ProductList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            $item['rec_start'] = !$item['is_hot'] || !$item['rec_start'] ? '无' : date('Y-m-d H:i:s', $item['rec_start']);
            $item['rec_end'] = !$item['is_hot'] || !$item['rec_end'] ? '无' : date('Y-m-d H:i:s', $item['rec_end']);
            //商家名称
            $item['tenant_name'] = SystemAdmin::where('id', '=', $item['tenant_id'])->value('real_name');
        }
        unset($item);
        if ($where['excel'] == 1) {
            $export = [];
    ...
    {
        return self::where('is_show', 1)->where('is_del', 0)->field('id,image,store_name')->select()->toArray();
    }

    public static function ProductListToExamine($where)
    {
        $model = self::getModelObjectExamine($where);
        if ($where['excel'] == 0) $model = $model->page((int) $where['page'], (int) $where['limit']);
        $data = ($data = $model->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item) {
            $cateName = CategoryModel::where('id', 'IN', $item['cate_id'])->column('cate_name', 'id');
            ...
            $item['sales_attr'] = self::getSales($item['id']); //属性销量
            $item['visitor'] = StoreVisit::where('product_id', $item['id'])->where('product_type', 'product')->count();
            $item['to_examine_name'] = '';
            if ($item['to_examine'] == 0) {
                $item['to_examine_name'] = "审核中";
            }
            if ($item['to_examine'] == 1) {
                $item['to_examine_name'] = "已通过";
            }
            if ($item['to_examine'] == -1) {
                $item['to_examine_name'] = "未通过";
            }
        ...
        $count = self::getModelObjectExamine($where)->count();
        return compact('count', 'data');
    }

    public static function getModelObjectExamine($where = [])
    {
        $model = new self();
        $model = $model->where('is_different', 1);
        if (!empty($where)) {
            $to_examine = $where['to_examine'] ?? 0;
            switch ((int) $to_examine) {
                case 1:
                    $model = $model->where('is_del', 0)->where('to_examine', 1);
                    break;
                case 2:
                    $model = $model->where('is_del', 0)->where('to_examine', 0);
                    break;
                case 3:
                    $model = $model->where('is_del', 0)->where('to_examine', -1);
                    break;
    ...
    /**
     * 获取产品列表-给产品数据分析页面提供数据
     * @param $where
     * @return array
     */
    public static function ProductListToProductLog($where)
    {
        $model = self::getModelObject($where);
        if ($where['excel'] == 0) $model = $model->page((int) $where['page'], (int) $where['limit']);
        $data = ($data = $model->select()) && count($data) ? $data->toArray() : [];
        ...
        foreach ($data as &$item) {
            ...
            $item['view_num'] = UserProductLog::getViewNumByProductId($item['id'], $start_time, $end_time);

            //曝光人数(UV)
            $item['view_user_num'] = UserProductLog::getViewUserNumByProductId($item['id'], $start_time, $end_time);

            //访问数
            $item['visit_num'] = UserProductLog::getVisitNumByProductId($item['id'], $start_time, $end_time);

            //访客数(IP)
            $item['visit_ip_num'] = UserProductLog::getVisitIpNumByProductId($item['id'], $start_time, $end_time);

            //浏览量(PV)
            $item['visit_user_num'] = UserProductLog::getVisitUserNumByProductId($item['id'], $start_time, $end_time);

            //加购人数
            $item['cart_user_num'] = UserProductLog::getCardUserNumByProductId($item['id'], $start_time, $end_time);

            //加购件数
            $item['cart_num'] = UserProductLog::getCardNumByProductId($item['id'], $start_time, $end_time);

            //支付人数
            $item['pay_user_num'] = UserProductLog::getPayUserNumByProductId($item['id'], $start_time, $end_time);

            //单品转换率=支付人数/访客数
            if ($item['visit_user_num'] > 0) {
                $rate = $item['pay_user_num'] / $item['visit_user_num'] * 100;
                $rate .= '%';
            } else {
                $rate = '0%';
            }
        ...
        if ($where['excel'] == 1) {
            ...
            PHPExcelService::setExcelHeader(['所属商家', '产品名称', '价格', '曝光次数', '曝光人数(UV)', '访问数', '访客数(IP)', '浏览量(PV)', '加购人数', '加购件数', '支付人数', '单品转换率'])
                ->setExcelTile('产品分析表', '产品信息' . time(), ' 生成时间：' . date('Y-m-d H:i:s', time()) . '   统计区间:' . date('Y-m-d', $start_time) . '  -  ' . date('Y-m-d', $end_time))
                ->setExcelContent($export)
                ->ExcelSave();
        }
        $count = self::getModelObject($where)->count();
