...
class SystemMenus extends BaseModel
{
    ...
    {
        $menusList = self::where('is_show_superadmin', '1')
            ->where('access', '1')
            ->order('sort DESC')
            ->select();
    ...
    public static function tidyMenuTier($adminFilter = false, $menusList, $pid = 0, $navList = [])
    {
        ...
        foreach ($menusList as $k => $menu) {
            ...
            if ($menu['pid'] == $pid) {
                ...
                //处理超级管理员的模块为superadmin
                $module=$menu['module']=='admin'?'superadmin':$menu['module'];
                $menu['url'] = !count($menu['child']) ? Url::buildUrl($module . '/' . $menu['controller'] . '/' . $menu['action'], $params) : 'javascript:void(0);';
