...
class SystemConfig extends BaseModel
{
     ...
     * @throws \think\exception\DbException
     */
    public static function getAll($id,$tenant_id =0 ,int $status = 1)
    {

        $where['config_tab_id'] = $id;
        $where['tenant_id'] = $tenant_id;
        if ($status == 1) $where['status'] = $status;
        //$list = self::where($where)->order('sort desc ,id asc')->select();
     ...
     * @return array
     */
    public static function getConfigTabAll($type = 0,$admin_id = 1)
    {
        $configAll = SystemConfigTab::getAll($type,$admin_id);
        $config_tab = [];
            ...
            switch ($data['type']) {
                ...
                case 'upload'://文件上传
                    switch ($data['upload_type']) {
                        case 1:
                            $data['value'] = json_decode($data['value'], true) ?: '';
                            $formbuider[] = Form::frameImageOne($data['menu_name'], $data['info'], Url::buildUrl('superadmin/widget.images/index', array('fodder' => $data['menu_name'])), $data['value'])->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                            break;
                        case 2:
                            $data['value'] = json_decode($data['value'], true) ?: [];
                            $formbuider[] = Form::frameImages($data['menu_name'], $data['info'], Url::buildUrl('superadmin/widget.images/index', array('fodder' => $data['menu_name'])), $data['value'])->maxLength(5)->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                            break;
                        case 3:
                            $data['value'] = json_decode($data['value'], true);
