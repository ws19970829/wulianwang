...
namespace app\superadmin\model\order;

use app\superadmin\model\system\SystemStore;
...
use app\superadmin\model\user\{User, UserBill};
use app\superadmin\model\ump\{StoreCouponUser, StorePink};
use crmeb\services\{PHPExcelService, WechatTemplateService};

/**
 * 订单管理Model
 * Class StoreOrder
 * @package app\superadmin\model\store
 */
class StoreOrder extends BaseModel
{
    ...
    public static function orderCount($orderType = '')
    {
        ...
        $data['qx'] = self::where(['is_system_del' => 0, 'is_del' => 0, 'status' => 4, 'refund_status' => 0])->count();
    ...
    public static function OrderList($where)
    {
        ...
        if (!empty($where['tenant_id'])) {
            $model = $model->where('a.tenant_id', $where['tenant_id']);
        }
        ...
        foreach ($data as &$item) {
            ...
            if ($item['status'] == 4 && $item['refund_status'] == 0) {
                ...
                if (!$item['plat_verify']) {
                    ...
                    <b style="color:#f124c7">申请退款：商家审核中</b><br/>
HTML;
                } else {
                    $item['status_name'] = <<<HTML
...
HTML;
        ...
        $count = self::getOrderWhere(
            ...
            //                ->where('(a.tenant_id='.session('tenant_id').') or (org_tenant_id='.session('tenant_id').')')
            ->count();
    ...
    public static function statusByWhere($status, $model = null, $alert = '')
    {
        ...
        if ('' === $status)
            ...
            return $model->where($alert . 'paid', 1)->where($alert . 'refund_status', 2)->where($alert . 'is_del', 0)->where($alert . 'status', 4);
        else if ($status == -3) //退款
        ...
        if (!empty($where['activity_type'])) {
            switch ($where['activity_type']) {
                case '1':
                    ...
                    break;
                case '2':
                    $model = $model->where('a.combination_id', '<>', '')
                        ->where('refund_status', 0)
                        ->where('is_del', 0)
                        ->where('is_system_del', 0);
                    break;
    ...
    public static function getOrderPrice($where)
    {
        ...
        if (isset($where['ordertype']) && $where['ordertype'] != 3) {
            $price['refund_price'] = self::getOrderWhere($where, $model)
                ->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])
                //                ->where('tenant_id',session('tenant_id'))
                ->sum('refund_price');
        } else {
            $price['refund_price'] = self::getOrderWhere($where, $model)
        ...
        if (isset($where['ordertype']) && $where['ordertype'] != 3) {
            $sumNumber = self::getOrderWhere($where, $model)->where($whereData)
                //                ->where('tenant_id',session('tenant_id'))
                ->field([
            ...
            $sumNumber = self::getOrderWhere($where, $model)
                ->where($whereData)
                ->where('is_consignment', 1)
                //                ->where('(tenant_id='.session('tenant_id').') or (org_tenant_id='.session('tenant_id').')')
                ->field([
        ...
        if (isset($where['ordertype']) && $where['ordertype'] != 3) {
            $list = self::getOrderWhere($where, $model)
                ->where($whereData)
                //                ->where('tenant_id',session('tenant_id'))
                ->group('pay_type')
                ->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
        } else {
            $list = self::getOrderWhere($where, $model)
                ->where($whereData)
                ->where('is_consignment', 1)
    ...
    public static function payTypeCount($orderType = 1)
    {
        ...
        // if ($orderType == 1) {        //普通订单
        //     $map = 'order_type=1 and is_consignment=0';
        // } else if ($orderType == 2) {      //异业订单
        //     $map = 'order_type=2';
        // } else {      //代销订单
        //     $map = 'is_consignment=1';
        // }
        $map = [];
        if (!empty(input('tenant_id'))) {
            $map[] = ['tenant_id', '=', input('tenant_id')];
        }
        ...
        $where['order'] = '';
        $where['pay_type'] = 1;
        $weixin = self::getOrderWhere($where, new self)
            //            ->where('tenant_id','=',$tenant_id)
            ->where($map)->count();
        $where['pay_type'] = 2;
        $zhifubao = self::getOrderWhere($where, new self)
            //            ->where('tenant_id','=',$tenant_id)
            ->where($map)
            ->count();
        $where['pay_type'] = 3;
        $offline = self::getOrderWhere($where, new self)
            //            ->where('tenant_id','=',$tenant_id)
            ->where($map)
            ->count();
     ...
     * @return float
     */
    public static function getIncomeByTenantID($tenant_id)
    {
     ...
     * @return float
     */
    public static function getRefundPriceByTenantID($tenant_id)
    {
        return self::where('tenant_id', '=', $tenant_id)
            ->where('pay_price', '>', 0)
            ->where('pay_time', '>', 0)
