...
HTML;
    ...
    public static function getOrderWhere($where, $model, $aler = '', $join = '')
    {
        ...
        if(!empty($where['tenant_id'])){
            $model = $model->where('tenant_id',$where['tenant_id']);
        }
    ...
    public static function payTypeCount()
    {
        ...
        $weixin = self::getOrderWhere($where, new self)->fetchSql(true)->count();

        $where['pay_type'] = 2;
        $zhifubao = self::getOrderWhere($where, new self)->count();
        //$where['pay_type'] = 3;
