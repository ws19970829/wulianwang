...
namespace app\superadmin\model\finance;

use app\models\store\StoreOrder;
use crmeb\traits\ModelTrait;
use crmeb\basic\BaseModel;
use app\models\user\UserBill;
use app\superadmin\model\user\User;
...
class FinanceModel extends BaseModel
{
    ...
    public function getOrderIdAttr($v, $data)
    {

        return StoreOrder::where('id', $data['link_id'])->value('order_id') ?? "";
    }
...
//            $where['add_time'] = array('between', [strtotime(date('Y/m') . '/01'), strtotime(date('Y/m') . '/' . date('t'))]);
    ...
    public static function getBillList($where)
    {
        ...
        $data = ($data = $whereModel->page((int) $where['page'], (int) $where['limit'])->select()) && count($data) ? $data->append(['order_id'])->toArray() : [];
    ...
    public static function setWhereList($where)
    {
        ...
        if ($where['order_id']) {
            $link_id = StoreOrder::where('order_id', $where['order_id'])->value('id');
            $model = $model->where('link_id', $link_id);
        }
        if ($where['account'] != '') {
            $model = $model->where('B.account', '=', $where['account']);
        }

        return $model->field(['A.*', 'FROM_UNIXTIME(A.add_time,"%Y-%m-%d %H:%i:%s") as add_time', 'B.uid', 'B.account']);
    }
