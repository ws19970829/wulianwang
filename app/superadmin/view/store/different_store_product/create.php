                            ...
                            <div class="layui-tab-item layui-show">
                                <div class="layui-row layui-col-space15">
                                    ...
                                    <div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
                                        <div class="grid-demo grid-demo-bg1">
                                            <div class="layui-form-item">
                                                <label class="layui-form-label">代销利润</label>
                                                <div class="layui-input-block">
                                                    <input style="width: 40%" type="number" name="consignment_price" lay-verify="title" autocomplete="off"
                                                           placeholder="请输入代销利润" class="layui-input" v-model="formData.consignment_price">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            ...
            goBack:function(){
                location.href = this.U({c:'store.DifferentStoreProduct',a:'index'});
            },
            U: function (opt) {
                var m = opt.m || 'superadmin', c = opt.c || window.controlle || '', a = opt.a || 'index', q = opt.q || '',
                    p = opt.p || {};
            ...
            getProductInfo: function () {
                var that = this;
                that.requestGet(that.U({m:"superadmin",c:"store.DifferentStoreProduct",a:'get_product_info',q:{id:that.id}})).then(function (res) {
            ...
            deleteAttrs: function (index) {
                var that = this;
                if(that.id > 0){
                    that.requestGet(that.U({c:"store.DifferentStoreProduct",a:'check_activity',q:{id:that.id}})).then(function (res) {
                        that.showMsg(res.msg);
                    }).catch(function (res) {
            ...
            generate: function (type = 0) {
                var that = this;
                this.requestPost(that.U({c:"store.DifferentStoreProduct",a:'is_format_attr',p:{id:that.id,type:type}}), {attrs:this.formData.items}).then(function (res) {
            ...
            handleSubmit:function () {
                ...
                if (that.formData.consignment_price <= 0) {
                    return that.showMsg('代销利润必需大于0');
                }
                ...
                that.requestPost(that.U({c:'store.DifferentStoreProduct',a:'save',p:{id:that.id}}),that.formData).then(function (res) {
                    that.confirm();
                }).catch(function (res) {
                    that.showMsg(res.msg);
                });
            ...
            confirm: function(){
                ...
                layui.use(['layer'], function () {
                    ...
                    layer.confirm(that.id ? '修改成功是否返回产品列表' : '添加成功是否返回产品列表', {
                        ...
                        location.href = that.U({c:'store.DifferentStoreProduct',a:'index'});
                    }, function(){
                        location.reload();
                    });
                });
    ...
    getRuleList:function (type) {
        var that = this;
        that.requestGet(that.U({c:'store.DifferentStoreProduct',a:'get_rule'})).then(function (res) {
