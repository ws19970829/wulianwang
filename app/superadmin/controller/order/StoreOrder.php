...
use app\superadmin\model\order\StoreOrderCartInfo;
use app\superadmin\model\system\Express;
...
use app\superadmin\model\order\StoreOrderStatus;
use app\superadmin\model\ump\StorePink;
...
use app\superadmin\model\order\StoreOrder as StoreOrderModel;
 ...
 * Class StoreOrder
 * @package app\superadmin\controller\store
 */
class StoreOrder extends AuthController
{
     ...
     * @Desc    异业订单列表
     */
    public function different_order_index()
    {
        $where = Util::getMore([
            ['status', ''],
            ['real_name', $this->request->param('real_name', '')],
     ...
     * @Desc    代销订单列表
     */
    public function consignment_order_index()
    {
        $where = Util::getMore([
            ['status', ''],
            ['real_name', $this->request->param('real_name', '')],
    ...
    public function del_order()
    {
        ...
        $res = StoreOrderModel::where('id', 'in', $ids)->update(['is_system_del' => 1, 'is_del' => 1]);
        if ($res)
    ...
    {
        $param = $request->param();
        $product = StoreOrderModel::get($param['id']);
        if (!$product) return Json::fail('数据不存在!');
        if ($product->refund_status == 2) {
            return Json::fail('该订单已经进行过退款交易!');
        }
        if (!$param['refund_status']) {
            $product->plat_verify = 0;
            $product->refund_status = 0;
            $product->save();
            (new UserMessage())->create_message($product['uid'], '通知', '订单号:' . $product['order_id'] . '退款失败！平台驳回');
            return Json::successful('修改成功!');
        }
        $out_trade_no = db('order_out_trade_no')
            ->where('order_id', $product['id'])
            ->order('id', 'desc')
            ->value('out_trade_no');
        if (empty($out_trade_no)) return Json::fail('该订单为余额支付，无法退款!');

        try {
            BaseModel::beginTrans();
            $product->status = 4;
            $product->refund_status = 2;
            $total = StoreOrderModel::where('id', 'in', function ($query) use ($out_trade_no) {
                $query->name('order_out_trade_no')->where('out_trade_no', $out_trade_no)->field('order_id');
            })->sum('pay_price');

            if ($product->pay_type == 'weixin') {
                (new WxpayController())->refund($out_trade_no, $total, $product['pay_price']);
            } elseif ($product->pay_type == 'zhifubao') {
                (new AlipayController())->app_refund($out_trade_no, $product['pay_price']);
            }
            $product->save();
            ...
            $res5 = true;
            $cartInfo = StoreOrderCartInfo::where('cart_id', 'in', $product['cart_id'])
                ->select()
                ->toArray();

            foreach ($cartInfo as $list) {
                if (!$cart = json_decode($list['cart_info'], true)) exception('json数据有误');
                //发货后退款，走线下退款流程
                if ($product['status'] == 1) continue;
                //无货预售，不补库存
                //if (!empty($cart['productInfo']['attrInfo']['advance_sale'])) continue;

                //增库存减销量
                if (!empty($cart['combination_id'])) {
                    $res5 = $res5 && StoreCombination::incCombinationStock($cart['cart_num'], $cart['combination_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                } elseif (!empty($cart['seckill_id'])) {
                    $res5 = $res5 && StoreSeckill::incSeckillStock($cart['cart_num'], $cart['seckill_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                } else {
                    $res5 = $res5 && StoreProduct::incProductStock($cart['cart_num'], $cart['product_id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '');
                }
            }
            $coupon_type = StoreCoupon::where('id', $product['coupon_id'])->value('type');
            $refund_price = $product['give_store_cash'];
            if ($coupon_type === 0) {
                $refund_price = $refund_price - $product['coupon_price'] < 0 ? 0 : bcsub($refund_price, $product['coupon_price']);
            }
            //退款账单明细记录
            UserBill::expend('支出', $product['uid'], 'now_money', 'pay_product_refund', $refund_price, $product['id'], 0, '用户退款' . floatval($refund_price) . '元', 1, 0);
            UserBill::expend('待结算金额', $product['uid'], 'now_money', 'pay_product_refund', $refund_price, $product['id'], 0,  '用户退款' . floatval($refund_price) . '元', 1, $product['tenant_id']);
            if (!$res5) {
                BaseModel::rollbackTrans();
                return Json::fail('扣减库存失败');
            }
            (new UserMessage())->create_message($product['uid'], '通知', '订单号:' . $product['order_id'] . '退款成功！');
