...
use app\superadmin\model\user\UserLevel as UserLevelModel;
use app\superadmin\model\system\{SystemUserLevel,SystemUserTask};
...
class UserLevel extends AuthController
{
    ...
    public function create($id = 0)
    {
        ...
        $field[] = Form::frameImageOne('icon', '图标', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'icon')), isset($vipinfo) ? $vipinfo->icon : '')->icon('image')->width('100%')->height('500px');
        $field[] = Form::frameImageOne('image', '会员背景', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')), isset($vipinfo) ? $vipinfo->image : '')->icon('image')->width('100%')->height('500px');
