...
//       $this->assign(SystemAttachmentModel::getAll($pid));
    ...
    public function upload()
    {
        ...
        $upload_type=(new PublicController())->getSysConfigValue('upload_type',1);
        $upload_type = $this->request->get('upload_type', $upload_type);
        try {
...
//            ]);
            ...
            //文件上传使用tenant_id为1的总平台配置
            $upload = new Upload((int)$upload_type, [
                'accessKey' => (new PublicController())->getSysConfigValue('accessKey',1),
                'secretKey' => (new PublicController())->getSysConfigValue('secretKey',1),
                'uploadUrl' => (new PublicController())->getSysConfigValue('uploadUrl',1),
                'storageName' => (new PublicController())->getSysConfigValue('storage_name',1),
                'storageRegion' => (new PublicController())->getSysConfigValue('storage_region',1),
            ]);
