...
use app\superadmin\model\system\{SystemAttachment, SystemGroupData, ShippingTemplates};
use app\superadmin\model\ump\{StoreSeckillAttr, StoreSeckillAttrResult, StoreSeckill as StoreSeckillModel, StoreSeckillTime};
use crmeb\services\{
    FormBuilder as Form,
    UtilService as Util,
    JsonService as Json
};
use app\superadmin\model\store\StoreCategory;
use think\Request;
...
class StoreSeckill extends AuthController
{
    ...
    public function index()
    {
        ...
        $shop = db('system_admin')->where('is_del', 0)->field('id,real_name')->select();
    ...
    public function get_seckill_list()
    {
        ...
        if (is_object($seckillList['list'])) $seckillList['list'] = $seckillList['list']->append(['status_text', 'shop_name'])->toArray();
        $data = $seckillList['list']['data'];

        foreach ($data as $k => $v) {
            $data[$k]['start_day'] = date('Y-m-d', $data[$k]['start_day']);
            $data[$k]['stop_day'] = date('Y-m-d', $data[$k]['stop_day']);
            if (!$data[$k]['rec_start'] || !$data[$k]['is_hot']) {
                $data[$k]['rec_start'] = '无';
            } else {
                $data[$k]['rec_start'] = date('Y-m-d H:i:s', $data[$k]['rec_start']);
            }

            if (!$data[$k]['rec_end'] || !$data[$k]['is_hot']) {
                $data[$k]['rec_end'] = '无';
            } else {
                $data[$k]['rec_end'] = date('Y-m-d H:i:s', $data[$k]['rec_end']);
            }
    ...
    public function rec(Request $request)
    {
        if ($request->isGet()) {
            ...
            foreach (explode(',', $ids) as $v) {
                $save[] = ['id' => $v, 'rec_start' => $rec_start, 'rec_end' => $rec_end, 'is_hot' => 1];
            }
            (new StoreSeckillModel)->saveAll($save);
    ...
    public function unrec(Request $request)
    {
        ...
        (new StoreSeckillModel)->whereIn('id', $ids)->update(['is_hot' => 0]);
        return Json::successful('取消推荐成功');
    }

    public function sort(Request $request)
    {
        StoreSeckillModel::where('id', $request->param('id'))->update(['sort' => $request->param('sort')]);
