...
use app\superadmin\model\system\{SystemAttachment, ShippingTemplates};
...
class StoreCombination extends AuthController
{
    ...
    public function get_combination_list()
    {
        ...
        foreach ($data as $k => $v) {
            ...
            if (!$data[$k]['rec_start'] || !$data[$k]['is_host']) {
                $data[$k]['rec_start'] = '无';
            } else {
                $data[$k]['rec_start'] = date('Y-m-d H:i:s', $data[$k]['rec_start']);
            }

            if (!$data[$k]['rec_end'] || !$data[$k]['is_host']) {
                $data[$k]['rec_end'] = '无';
            } else {
    ...
    public function combination($id = 0)
    {
        ...
        $f[] = Form::frameImageOne('image', '商品主图片(305*305px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImages('images', '商品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'images')), json_decode($product->getData('slider_image')))->maxLength(5)->icon('images')->width('100%')->height('500px');
    ...
    public function create()
    {
        ...
        $f[] = Form::dateTimeRange('section_time', '拼团时间');
        $f[] = Form::frameImageOne('image', '商品主图片(305*305px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImages('images', '商品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'images')))->maxLength(5)->icon('images')->width('100%')->height('500px');
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::frameImageOne('image', '商品主图片(305*305px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImages('images', '商品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'images')), json_decode($product->getData('images')))->maxLength(5)->icon('images')->width('100%')->height('500px');
    ...
    public function rec(Request $request)
    {
        if ($request->isGet()) {
            ...
            foreach (explode(',', $ids) as $v) {
                $save[] = ['id' => $v, 'rec_start' => $rec_start, 'rec_end' => $rec_end, 'is_host' => 1];
            }
            (new StoreCombinationModel)->saveAll($save);
    ...
    public function unrec(Request $request)
    {
        ...
        (new StoreCombinationModel)->whereIn('id', $ids)->update(['is_host' => 0]);
        return Json::successful('取消推荐成功');
    }

    public function sort(Request $request)
    {
        StoreCombinationModel::where('id', $request->param('id'))->update(['sort' => $request->param('sort')]);
