...
use app\superadmin\model\ump\StoreBargain as StoreBargainModel;
...
class StoreBargain extends AuthController
{
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::frameImages('images', '商品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'images')), json_decode($product->getData('images'), 1))->maxLength(5)->icon('images')->width('100%')->height('500px');
    ...
    public function bargain($id = 0)
    {
        ...
        $f[] = Form::dateTimeRange('section_time', '活动时间');//->format("yyyy-MM-dd HH:mm:ss");
        $f[] = Form::frameImageOne('image', '商品主图片(305*305px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImages('images', '商品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'images')), json_decode($product->getData('slider_image'), 1))->maxLength(5)->icon('images')->width('100%')->height('500px');
