...
use app\superadmin\model\order\StoreOrder as StoreOrderModel; //订单
use app\superadmin\model\system\{SystemConfig, SystemMenus, SystemRole};
use app\superadmin\model\user\{User, UserExtract as UserExtractModel, User as UserModel};
use app\superadmin\model\store\{StoreProduct, StoreProductReply as StoreProductReplyModel, StoreProduct as ProductModel};
...
class Index extends AuthController
{
    ...
    public function main()
    {
        ...
        $topData['stockProduct'] = StoreProduct::where('stock', '<=', $replenishment_num)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('is_show', 1)
            ->where('is_del', 0)
            ->count();
        ...
        $pre_day_order_p = StoreOrderModel::where('paid', 1)
            ->where('tenant_id', '=', session('tenant_id'))
            ->where('pay_time', '>', $pre_day)
            ->where('pay_time', '<', $now_day)
            ->count();
        $first_line['d_num'] = [
        ...
        $pre_month_order_p = StoreOrderModel::where('paid', 1)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('pay_time', '>', $beforyester_day)
            ->where('pay_time', '<', $pre_day)
            ->sum('pay_price');
        $first_line['d_price'] = [
        ...
        $pre_month_order_p = StoreOrderModel::where('paid', 1)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('pay_time', '>', $pre_month)
            ->where('pay_time', '<', $now_month)
            ->value('sum(pay_price)');
        $first_line['m_price'] = [
            'data' => $now_month_order_p > 0 ? $now_month_order_p : 0,
            'percent' => abs($now_month_order_p - $pre_month_order_p),
        ...
        //新粉丝->日
        $now_day_user = User::where('add_time', '>', $now_day)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->count();
        $pre_day_user = User::where('add_time', '>', $pre_day)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('add_time', '<', $now_day)
            ->count();
        ...
        //新粉丝->月
        $now_month_user = User::where('add_time', '>', $now_month)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->count();
        $pre_month_user = User::where('add_time', '>', $pre_month)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('add_time', '<', $now_month)
            ->count();
        $first_line['month'] = [
        ...
        $now_order_info_c = StoreOrderModel::where('add_time', '>', $pre_month)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('add_time', '<', $now_month)
            ->count();
        $pre_order_info_c = StoreOrderModel::where('add_time', '>', $second_pre_month)
            //            ->where('tenant_id','=',session('tenant_id'))
            ->where('add_time', '<', $second_now_month)
            ->count();
        $order_info["second"] = [
            'data' => $now_order_info_c ? $now_order_info_c : 0,
            'percent' => abs($now_order_info_c - $pre_order_info_c),
    ...
    public function Jnotice($newTime = 30)
    {
        header('Content-type:text/json');
        $data = [];
        $data['ordernum'] = StoreOrderModel::statusByWhere(1)
            ->count(); //待发货
        ...
        $data['refund'] = StoreOrderModel::where('is_del', 0)
            ->where('refund_status', 1)
            //->where('plat_verify',1)
            ->where('is_system_del', 0)
            ->count();
        // $data['reflectnum'] = UserExtractModel::where('status', 0)->count();; //提现
        $data['msgcount'] = intval($data['ordernum']) + intval($data['inventory']) + intval($data['refund']);
        //新订单提醒
