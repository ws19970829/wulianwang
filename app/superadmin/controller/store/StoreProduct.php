...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        $shop_id = input('tenant_id');
        $this->assign(compact('type', 'onsale', 'forsale', 'warehouse', 'outofstock', 'policeforce', 'recycle', 'shop','shop_id'));
        return $this->fetch();
    }
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::frameImageOne('image', '产品主图片(305*305px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('slider_image', '产品轮播图(640*640px)', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'slider_image')), json_decode($product->getData('slider_image'), 1) ?: [])->maxLength(5)->icon('images')->width('100%')->height('500px'),
    ...
    public function is_format_attr($id = 0, $type = 0)
    {
        ...
        foreach ($value as $key => $item) {
            ...
            $types = 1;
            if ($id) {
                $sukValue = StoreProductAttrValue::where('product_id', $id)->where('type', 0)->where('suk', $suk)->column('bar_code,cost,price,ot_price,stock,image as pic,weight,volume,brokerage,brokerage_two,discount,discount_gt,discount_lt,advance_sale,advance_day,advance_time', 'suk');
                if (!count($sukValue)) {
    ...
    public function unrec(Request $request)
    {
        ...
        db('store_product')->whereIn('id', $ids)->update(['is_hot' => 0]);
