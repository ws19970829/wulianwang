...
class SystemConfig extends AuthController
{
    ...
    public function index()
    {
        ...
        if ($type == 3) {//其它分类
            ...
            $config_tab = ConfigModel::getConfigTabAll($type,$admin_id);//获取一级tab
        }
        ...
        //获取分类配置参数
        $list = ConfigModel::getAll($tid,$admin_id);
        ...
        switch ($menu['type']) {
            ...
            case 'upload':
                if ($menu['upload_type'] == 1) {
                    $menu['value'] = json_decode($menu['value'], true);
                    $formbuider[] = Form::frameImageOne('value', '图片', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'value')), (string)$menu['value'])->icon('image')->width('100%')->height('550px');
                } elseif ($menu['upload_type'] == 2) {
                    $menu['value'] = json_decode($menu['value'], true) ?: [];
                    $formbuider[] = Form::frameImages('value', '多图片', Url::buildUrl('superadmin/widget.images/index', array('fodder' => 'value')), $menu['value'])->maxLength(5)->icon('images')->width('100%')->height('550px')->spin(0);
                } else {
                    $menu['value'] = json_decode($menu['value'], true);
