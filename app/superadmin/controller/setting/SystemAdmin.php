...
use app\models\user\WechatUser as WechatUserModel;
...
class SystemAdmin extends AuthController
{
    ...
    public function save()
    {
        ...
        if (!$res) {
            return Json::fail('添加管理员失败');
        }

        //前台登录用户信息
        $user_data = [
            'account_type' => 2,
            'admin_account_id' => $res->id,
        ...
        WechatUserModel::create($wechat_user_data);
    ...
    public function update($id)
    {
        ...
        if (isset($data['pwd'])) {
            $user_data['pwd'] = $data['pwd'];
        }

        UserModel::where('admin_account_id', $id)->update($user_data);
