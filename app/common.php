...
if (!function_exists('sys_config')) {
    ...
    function sys_config(string $name, $default = '')
    {
        ...
        $config = trim(db('system_config')->where('tenant_id', 1)->where('menu_name', $name)->value('value'), '""');
...
function mylog($mark, $log_content, $keyp = "")
{
    $max_size = 30000000;
    if ($keyp == "") {
        $log_filename = root_path() . 'tlogs/' . date('Ym-d') . ".log";
    } else {
        $log_filename = root_path() . 'tlogs/' . $keyp . ".log";
    }
...
function getPid($category, $pid)
{
    static $tree = [];
    foreach ($category as $item) {
        if ($item['id'] == $pid) {
            $pid = db('store_category')->where('id', $item['pid'])->value('id');
            getPid($category, $pid);
            $tree[] = $item['id'];
        }
    }
 ...
 function getMenuTree($cate, $parent_id = 0, $level = 0)
{
    ...
    foreach($cate as $key => $value)
    {
        ...
        {
            $arrTree[] = $value['id'];
            unset($cate[$key]); //注销当前节点数据，降低已没用的遍历
            getMenuTree($cate, $value[ 'id'], $level);
        }
    }
    return $arrTree;
}
