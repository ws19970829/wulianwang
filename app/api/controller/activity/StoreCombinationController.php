...
class StoreCombinationController
{
    ...
    public function lst(Request $request)
    {
        ...
        $combinationList = StoreCombination::getAll($page, $limit, $condition);
        if (!count($combinationList)) return app('json')->successful([]);
        $count = $combinationList->count();
        $list = $combinationList->append(['goods_url','goods_num_text','quota_text','price_text','ot_price_text'])->hidden(['info', 'product_id', 'images', 'mer_id', 'attr', 'sort', 'stock', 'sales', 'add_time', 'is_del', 'is_show', 'browse', 'cost', 'is_show',  'postage', 'is_postage', 'is_host', 'mer_use', 'combination'])->toArray();
        if(!empty($list)){
            foreach($list as &$v){
